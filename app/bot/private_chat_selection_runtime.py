    ...
    *,
    ...
    if isinstance(search_service, tg.SearchMediaService) and chat_id is not None:
        ...
        if clarification_pending:
            ...
            return True
        media_candidate_selection = search_service.is_media_candidate_selection(chat_id, int(query))
        if media_candidate_selection is None:
            await reply_func(tg.SERVICE_NOT_READY_TEXT)
            return True
        if media_candidate_selection:
            reply = await execution_gate.run(
                tg.ACTION_SEARCH_MEDIA,
                lambda: search_service.search_resources_for_selected_media(
                    chat_id,
                    query,
                    channel=channel,
                ),
