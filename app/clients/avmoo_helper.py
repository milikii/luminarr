...
AvmooReadOnlyMatch = JavLibraryReadOnlyMatch
...
class AvmooReadOnlyHelperClient:
    def __init__(
        self,
    ...
    async def lookup(self, lookup_text: str) -> AvmooReadOnlyMatch | None:
...
def _build_search_url(display_id: str) -> str:
    ...
    return f"https://avmoo.shop/cn/search/{encoded_query}"


def _normalize_expected_display_id(lookup_text: str) -> str:
    content_match = extract_exact_adult_content_match(lookup_text, source_site="avmoo")
...
def _resolve_detail_url(*, response: httpx.Response, expected_display_id: str) -> str:
    ...
    if _is_detail_url(response_url):
        return response_url
...
def _extract_read_only_match(
    *,
    ...
    detail_url: str,
) -> AvmooReadOnlyMatch | None:
    detail_id = _extract_detail_id(html) or expected_display_id
    title = _extract_detail_title(html) or detail_id
    content_match = extract_exact_adult_content_match(f"{detail_id} {title}", source_site="avmoo")
    if (
...
def _extract_detail_id(html: str) -> str:
    ...
    content_match = extract_exact_adult_content_match(detail_id, source_site="avmoo")
...
def _looks_like_expected_display_id(text: str, *, expected_display_id: str) -> bool:
    if not text:
        return False
    content_match = extract_exact_adult_content_match(text, source_site="avmoo")
...
def _is_detail_url(value: str) -> bool:
    return "/movie/" in urlparse(value).path.lower()
