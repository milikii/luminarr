...
class StaticAdultMovieBoxHelperClient:
    """Read exact-ID adult metadata from static movie-box style sites."""
    ...
    def __init__(
        ...
        *,
        ...
        self._search_path_template = search_path_template
        self._detail_path_markers = tuple(marker.lower() for marker in detail_path_markers if marker)
        self._timeout_seconds = timeout_seconds
        self._proxy_url = proxy_url.strip()

    async def lookup(self, lookup_text: str) -> JavLibraryReadOnlyMatch | None:
        """Return a metadata match for an exact censored ID, or None when not safely matched."""
        expected_display_id = self._normalize_expected_display_id(lookup_text)
        if not expected_display_id:
            return None

        search_url = self._build_search_url(expected_display_id)
        response = await self._get(search_url)
        detail_url = _resolve_detail_url(
            response=response,
            expected_display_id=expected_display_id,
            base_url=self._base_url,
            detail_path_markers=self._detail_path_markers,
        )
        if not detail_url:
            return None
    ...
    def _build_search_url(self, display_id: str) -> str:
        encoded_query = quote_plus(display_id)
        path = self._search_path_template.format(query=encoded_query)
        return f"{self._base_url}{path}"

    def _normalize_expected_display_id(self, lookup_text: str) -> str:
        content_match = extract_exact_adult_content_match(lookup_text, source_site=self._source_site)
...
def _resolve_detail_url(
    *,
    ...
    if _is_detail_url(response_url, expected_display_id=expected_display_id, detail_path_markers=detail_path_markers):
        return response_url
...
def _extract_read_only_match(
    *,
    ...
    detail_id = _extract_detail_id(html, source_site=source_site) or expected_display_id
    title = _extract_detail_title(html) or detail_id
    content_match = extract_exact_adult_content_match(f"{detail_id} {title}", source_site=source_site)
    if (
...
def _extract_detail_id(html: str, *, source_site: str) -> str:
    detail_id = _extract_detail_field(html, "识别码", "識別碼", "品番", "番号", "ID")
    content_match = extract_exact_adult_content_match(detail_id, source_site=source_site)
...
def _extract_detail_genres(html: str) -> tuple[str, ...]:
    ...
    values = tuple(
        ...
        for value in re.findall(
            r"""<a\b[^>]*\bclass=["'][^"']*\bgenre\b[^"']*["'][^>]*>(?P<text>.*?)</a>""",
            html,
...
def _extract_attr(attrs: str, attr_name: str) -> str:
    matched = re.search(rf"""\b{re.escape(attr_name)}\s*=\s*["'](?P<value>[^"']+)["']""", attrs, flags=re.IGNORECASE)
    if matched is None:
        return ""
...
def _has_html_class(attrs: str, class_name: str) -> bool:
    ...
    if matched is None:
        return False
    return class_name in str(matched.group("classes") or "").split()


def _split_metadata_values(value: str) -> tuple[str, ...]:
    if not value:
        return ()
    return _dedupe_texts(part.strip() for part in re.split(r"[,，/ ]+", value) if part.strip())
...
def _normalize_label(value: str) -> str:
    return _clean_html_text(value).rstrip(":：").replace(" ", "")


def _looks_like_expected_display_id(text: str, *, expected_display_id: str) -> bool:
    if not text:
        return False
    content_match = extract_exact_adult_content_match(text)
    if content_match is not None:
        return content_match.display_id == expected_display_id
    return expected_display_id.lower() in text.strip().lower()


def _is_detail_url(value: str, *, expected_display_id: str, detail_path_markers: tuple[str, ...]) -> bool:
    ...
    if "/search/" in path:
        return False
    return _looks_like_expected_display_id(path, expected_display_id=expected_display_id)
