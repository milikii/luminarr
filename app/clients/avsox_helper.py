...
AvsoxReadOnlyMatch = JavLibraryReadOnlyMatch
...
class AvsoxReadOnlyHelperClient:
    def __init__(
        self,
    ...
    async def lookup(self, lookup_text: str) -> AvsoxReadOnlyMatch | None:
...
def _build_search_url(display_id: str) -> str:
    ...
    return f"https://avsox.click/cn/search/{encoded_query}"


def _normalize_expected_display_id(lookup_text: str) -> str:
    content_match = extract_exact_adult_content_match(lookup_text, source_site="avsox")
...
def _extract_read_only_match(
    *,
    ...
    detail_url: str,
) -> AvsoxReadOnlyMatch | None:
    detail_id = _extract_detail_id(html) or expected_display_id
    title = _extract_detail_title(html) or detail_id
    content_match = extract_exact_adult_content_match(f"{detail_id} {title}", source_site="avsox")
    if (
...
def _extract_detail_id(html: str) -> str:
    ...
    content_match = extract_exact_adult_content_match(detail_id, source_site="avsox")
...
def _is_detail_url(url: str) -> bool:
    parsed = urlparse(url)
    return parsed.path.startswith("/cn/movie/")
