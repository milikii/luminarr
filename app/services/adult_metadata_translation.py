...
class AdultMetadataTranslatorService:
    ...
    async def translate_candidates(
        ...
        requests = build_adult_metadata_translation_requests(cloned_candidates)
        if not requests:
            return cloned_candidates
        if not self._api_key:
            emit_operational_log(
                title="成人 metadata 翻译未启用",
                detail=f"request_count={len(requests)} 原因=missing api key",
                fix_hint="补齐 SUBTITLE_TRANSLATION_API_KEY；当前会保留原始成人 metadata，不影响资源卡片展示。",
            )
            return cloned_candidates
        results = await self.translate_requests(requests)
        if not results:
            return cloned_candidates
        if not _has_complete_translation_results(requests=requests, results=results):
            emit_operational_log(
                title="成人 metadata 翻译结果不完整",
                detail=(
                    f"request_ids={_join_request_ids(request.request_id for request in requests)} "
                    f"result_ids={_join_request_ids(result.request_id for result in results)}"
                ),
                fix_hint="检查成人 metadata 翻译 request_id 对齐与响应完整性；当前会保留原始成人 metadata，不应用部分翻译结果。",
            )
            return cloned_candidates
        return apply_adult_metadata_translation_results(cloned_candidates, results)
