        ...
        *,
        ...
        if self._job_repo is not None and context.job.state == JOB_STATE_PENDING_APPROVAL:
            try:
                ...
                if not cancelled:
                    print(
                        f"\033[31m[下载确认超时任务取消失败]\033[0m task_ref={task_ref} job_id={context.job.job_id} task_id={context.job.task_id} task_hash={context.job.task_hash} version={context.job.version} 错误=jobs.cancel_pending_job returned False\n\033[33m[处理建议]\033[0m 检查 SQLite/jobs 表更新是否正常；当前会继续返回超时文本，但任务真相可能仍残留在待确认状态。",
                        flush=True,
                    )
        self._clear_pending_context(chat_id=chat_id, task_ref=task_ref)
        self._record_event(
