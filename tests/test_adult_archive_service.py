...
def test_adult_archive_service_raises_operation_error_when_cleanup_remove_fails(tmp_path: Path) -> None:
    database = SqliteDatabase(str(tmp_path / "adult.sqlite3"))
    database.initialize()
    registry_repo = AdultContentRegistryRepo(database)
    event_repo = JobEventRepo(database)
    ...
    source_file = source_dir / "SSIS-123.mp4"
    source_file.write_bytes(b"adult-video")
    archive_path = tmp_path / "archive" / "censored" / "SSIS-123.mp4"
    archive_path.parent.mkdir(parents=True)
    archive_path.write_bytes(b"adult-video")
    registry_repo.mark_downloading(
        normalized_content_id="censored:ssis-123",
        content_id_kind="censored",
        archive_category="censored",
    ...
    async def failing_remove_torrent(task_ref: str, chat_id: int | None = None, delete_local_data: bool = True) -> None:
        _ = (task_ref, chat_id, delete_local_data)
        raise ValueError("downloader route missing")

    service = AdultArchiveService(
        get_import_source_func=fake_get_import_source,
        remove_torrent_func=failing_remove_torrent,
        registry_repo=registry_repo,
        job_event_repo=event_repo,
        archive_destinations=(
            AdultArchiveDestination(category="censored", label="有码", target_dir=str(archive_path.parent)),
        ),
        retention_hours=0,
    )
    ...
    with pytest.raises(AdultArchiveOperationError, match="cleanup downloader removal failed"):
        asyncio.run(
            service.run_for_record(
                candidate=record,
                registry_record=registry_repo.get_by_content_id(normalized_content_id="censored:ssis-123"),
    ...
    assert source_file.exists()
    assert event_repo.list_events_for_task_identity(task_id="123", task_hash="hash-123") == []
...
def test_adult_archive_service_raises_state_unavailable_when_cleanup_registry_persist_fails(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    database = SqliteDatabase(str(tmp_path / "adult.sqlite3"))
    ...
    registry_repo.mark_archived_present(
        normalized_content_id="censored:ssis-123",
        archive_path=str(tmp_path / "archive" / "SSIS-123.mp4"),
        task_id="123",
        task_hash="hash-123",
    )
    ...
    monkeypatch.setattr(registry_repo, "mark_archived_deleted", _raise_persist_error)
    service = AdultArchiveService(
        get_import_source_func=fake_get_import_source,
        remove_torrent_func=fake_remove_torrent,
        registry_repo=registry_repo,
    ...
    with pytest.raises(AdultArchiveStateUnavailableError, match="adult archive cleanup state persist failed"):
        asyncio.run(
            service.run_for_record(
                candidate=retained_record,
                registry_record=registry_repo.get_by_content_id(normalized_content_id="censored:ssis-123"),
