...
def test_get_status_text_warns_when_download_monitor_readback_row_is_corrupted(capsys) -> None:
    monitor_repo = type(
        "CorruptedRecordRepo",
        (),
        {
...
def test_get_status_text_returns_state_unavailable_when_skip_event_row_is_corrupted(
    tmp_path: Path, capsys
) -> None:
    database = SqliteDatabase(str(tmp_path / "state.sqlite3"))
    database.initialize()
...
def test_post_download_auto_import_run_once_marks_state_unavailable_when_skip_event_row_is_corrupted(
    tmp_path: Path, capsys
) -> None:
    database = SqliteDatabase(str(tmp_path / "state.sqlite3"))
    database.initialize()
...
def test_post_download_auto_import_run_for_record_stops_when_adult_archive_state_is_unavailable(
    tmp_path: Path,
    capsys,
) -> None:
    database = SqliteDatabase(str(tmp_path / "adult.sqlite3"))
    ...
    adult_archive_service = type(
        "AdultArchiveService",
        (),
        {"run_for_record": AsyncMock(side_effect=AdultArchiveStateUnavailableError("persist failed"))},
    )()
    auto_import_service = PostDownloadAutoImportService(
        download_monitor_repo=None,
        job_event_repo=type("EventRepo", (), {"list_events_for_task_identity": lambda self, **kwargs: []})(),
    ...
    with pytest.raises(AutoImportStateUnavailableError, match="adult archive state unavailable"):
        asyncio.run(auto_import_service.run_for_record(record))
...
def test_post_download_auto_import_run_for_record_raises_when_skip_event_row_is_corrupted(capsys) -> None:
    auto_import = AsyncMock(return_value="不应走到这里")
    auto_import_service = PostDownloadAutoImportService(
        download_monitor_repo=None,
        job_event_repo=type(
