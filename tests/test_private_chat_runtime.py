...
def test_dispatch_private_chat_text_replies_service_not_ready_when_bt_processing_path_clear_result_is_missing_on_cancel(
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_dispatch_private_chat_text_replies_service_not_ready_when_bt_classification_clear_fails_on_cancel(
    ...
    class _FailingPendingRepo(BtPendingRepo):
        def clear_pending(self, *, chat_id: int, expected_stage: str | None = None) -> bool:
            if expected_stage == "classification":
                raise sqlite3.OperationalError("db down")
            return False
...
def test_dispatch_private_chat_text_replies_service_not_ready_when_bt_classification_clear_result_is_missing_on_cancel(
    capsys: pytest.CaptureFixture[str],
) -> None:
    class _MissingClearResultPendingRepo(BtPendingRepo):
        def clear_pending(self, *, chat_id: int, expected_stage: str | None = None):
...
def test_dispatch_private_chat_text_replies_service_not_ready_when_bt_tmdb_clear_fails_on_cancel(
    capsys: pytest.CaptureFixture[str],
) -> None:
    class _FailingPendingRepo(BtPendingRepo):
        def clear_pending(self, *, chat_id: int, expected_stage: str | None = None) -> bool:
        ...
        dispatch_private_chat_text(
            ...
            | {
                ...
                "bt_tmdb_association_pending_by_chat": {
                    1001: SimpleNamespace(media_kind="movie", source="magnet:?xt=urn:btih:abcdef1234567890")
                },
            },
        )
    )
...
def test_dispatch_private_chat_text_replies_service_not_ready_when_bt_tmdb_clear_fails_before_media_import_flow(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    class _FailingPendingRepo(BtPendingRepo):
