...
def test_handle_message_bt_batch_preview_sort_page_number_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
    ...
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?s=seeders&o=desc&p=2" in sent_text
    assert "title-Frieren S01E13" in sent_text
    assert "当前预览范围：1" in sent_text
...
def test_handle_message_bt_batch_preview_category_sort_page_number_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
    ...
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?c=1_2&s=seeders&o=desc&p=2" in sent_text
    assert "title-Frieren S01E15" in sent_text
    assert "当前预览范围：1" in sent_text
...
def test_handle_message_bt_batch_preview_user_sort_page_number_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
...
def test_handle_message_bt_batch_preview_page_number_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
    ...
    update, reply_text = _build_update("bt批量 https://nyaa.si/?f=0&c=1_2&u=subsplease&p=2 1-1")
    search_service = SearchMediaService(
        _fake_search,
        raw_search_func=unexpected_raw_search,
        raw_page_search_func=fake_page_search,
    ...
    reply_text.assert_awaited_once()
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?f=0&c=1_2&u=subsplease&p=2" in sent_text
    assert "title-Frieren S01E11" in sent_text
    assert "当前预览范围：1" in sent_text


def test_handle_message_bt_batch_preview_uncategorized_user_page_number_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
...
def test_handle_message_bt_batch_preview_category_search_page_number_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
    ...
    update, reply_text = _build_update("bt批量 https://nyaa.si/?c=1_2&q=frieren&p=2 1-1")
    search_service = SearchMediaService(
        _fake_search,
        raw_search_func=unexpected_raw_search,
        raw_page_search_func=fake_page_search,
    ...
    reply_text.assert_awaited_once()
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?c=1_2&q=frieren&p=2" in sent_text
    assert "title-Frieren S01E24" in sent_text
    assert "当前预览范围：1" in sent_text
...
def test_handle_message_bt_batch_preview_category_search_exact_base_page_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
    ...
    update, reply_text = _build_update("bt批量 https://nyaa.si/?c=1_2&q=frieren 1-1")
    search_service = SearchMediaService(
        _fake_search,
        raw_search_func=unexpected_raw_search,
        raw_page_search_func=fake_page_search,
    ...
    reply_text.assert_awaited_once()
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?c=1_2&q=frieren" in sent_text
    assert "title-Frieren S01E30" in sent_text
    assert "当前预览范围：1" in sent_text
...
def test_handle_message_bt_batch_preview_home_pagination_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist home pagination url")

    async def fake_page_search(page_url: str) -> list[dict[str, object]]:
        assert page_url == "https://nyaa.si/?p=2"
        return [
            {
                "title": "title-Frieren S01E34",
                "source": "magnet:?xt=urn:btih:3434343434343434343434343434343434343434",
                "infoHash": "3434343434343434343434343434343434343434",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            }
        ]

    update, reply_text = _build_update("bt批量 https://nyaa.si/?p=2 1-1")
    search_service = SearchMediaService(
        _fake_search,
        raw_search_func=unexpected_raw_search,
        raw_page_search_func=fake_page_search,
    ...
    reply_text.assert_awaited_once()
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?p=2" in sent_text
    assert "title-Frieren S01E34" in sent_text
    assert "当前预览范围：1" in sent_text


def test_handle_message_bt_batch_preview_category_list_pagination_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist category list pagination url")

    async def fake_page_search(page_url: str) -> list[dict[str, object]]:
        assert page_url == "https://nyaa.si/?f=0&c=1_2&p=2"
        return [
            {
                "title": "title-Frieren S01E36",
                "source": "magnet:?xt=urn:btih:3636363636363636363636363636363636363636",
                "infoHash": "3636363636363636363636363636363636363636",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            }
        ]

    update, reply_text = _build_update("bt批量 https://nyaa.si/?f=0&c=1_2&p=2 1-1")
    search_service = SearchMediaService(
        _fake_search,
        raw_search_func=unexpected_raw_search,
        raw_page_search_func=fake_page_search,
    ...
    reply_text.assert_awaited_once()
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?f=0&c=1_2&p=2" in sent_text
    assert "title-Frieren S01E36" in sent_text
    assert "当前预览范围：1" in sent_text


def test_handle_message_bt_batch_preview_category_base_pagination_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist category base pagination url")

    async def fake_page_search(page_url: str) -> list[dict[str, object]]:
        assert page_url == "https://nyaa.si/?c=1_2&p=2"
        return [
            {
                "title": "title-Frieren S01E38",
                "source": "magnet:?xt=urn:btih:3838383838383838383838383838383838383838",
                "infoHash": "3838383838383838383838383838383838383838",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            }
        ]

    update, reply_text = _build_update("bt批量 https://nyaa.si/?c=1_2&p=2 1-1")
    search_service = SearchMediaService(
        _fake_search,
        raw_search_func=unexpected_raw_search,
        raw_page_search_func=fake_page_search,
    ...
    reply_text.assert_awaited_once()
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?c=1_2&p=2" in sent_text
    assert "title-Frieren S01E38" in sent_text
    assert "当前预览范围：1" in sent_text
...
def test_handle_message_bt_batch_preview_search_sort_page_number_url_routes_to_page_fetch() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
    ...
    sent_text = reply_text.await_args.args[0]
    assert "BT 批量预览结果：https://nyaa.si/?f=0&c=1_2&q=frieren&s=seeders&o=desc&p=2" in sent_text
    assert "title-Frieren S01E23" in sent_text
    assert "当前预览范围：1" in sent_text
...
def test_handle_message_bt_batch_confirm_reuses_sort_page_number_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist sort page number url")
...
def test_handle_message_bt_batch_confirm_reuses_sort_page_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
    ...
    async def fake_page_search(page_url: str) -> list[dict[str, object]]:
        ...
        return [
            {
                "title": "title-Frieren S01E05",
                "source": "magnet:?xt=urn:btih:0505050505050505050505050505050505050505",
                "infoHash": "0505050505050505050505050505050505050505",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            },
            {
                ...
                "infoHash": "0606060606060606060606060606060606060606",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            },
        ]
    ...
    preview_update, preview_reply = _build_update("bt批量 https://nyaa.si/?s=seeders&o=desc 1-2")
    asyncio.run(handle_message(preview_update, context))
    assert preview_reply.await_count == 1
    ...
    sent_text = confirm_reply.await_args.args[0]
    assert sent_text.count("待确认：下载 ⏳") == 2
    assert "片名：title-Frieren S01E05" in sent_text
...
def test_handle_message_bt_batch_confirm_reuses_user_sort_page_number_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist user sort page number url")
...
def test_handle_message_bt_batch_confirm_reuses_page_number_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist page number url")
    ...
    preview_update, preview_reply = _build_update("bt批量 https://nyaa.si/?f=0&c=1_2&u=subsplease&p=2 1-2")
    asyncio.run(handle_message(preview_update, context))
    assert preview_reply.await_count == 1
...
def test_handle_message_bt_batch_confirm_reuses_uncategorized_user_page_number_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for uncategorized user page number url")
...
def test_handle_message_bt_batch_confirm_reuses_category_search_page_number_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist category search page number url")
    ...
    preview_update, preview_reply = _build_update("bt批量 https://nyaa.si/?c=1_2&q=frieren&p=2 1-2")
    asyncio.run(handle_message(preview_update, context))
    assert preview_reply.await_count == 1
...
def test_handle_message_bt_batch_confirm_reuses_category_search_exact_base_page_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist category search exact base page")
    ...
    preview_update, preview_reply = _build_update("bt批量 https://nyaa.si/?c=1_2&q=frieren 1-2")
    asyncio.run(handle_message(preview_update, context))
    assert preview_reply.await_count == 1
...
def test_handle_message_bt_batch_confirm_reuses_home_pagination_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist home pagination url")
    ...
    async def fake_page_search(page_url: str) -> list[dict[str, object]]:
        ...
        return [
            ...
            {
                "title": "title-Frieren S01E35",
                "source": "magnet:?xt=urn:btih:3535353535353535353535353535353535353535",
                "infoHash": "3535353535353535353535353535353535353535",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            },
        ]
    ...
    preview_update, preview_reply = _build_update("bt批量 https://nyaa.si/?p=2 1-2")
    asyncio.run(handle_message(preview_update, context))
    assert preview_reply.await_count == 1
    ...
    sent_text = confirm_reply.await_args.args[0]
    assert sent_text.count("待确认：下载 ⏳") == 2
    assert "片名：title-Frieren S01E34" in sent_text
    assert "片名：title-Frieren S01E35" in sent_text
    assert "确认下载：发送 confirm 1" in sent_text
    assert "确认下载：发送 confirm 2" in sent_text


def test_handle_message_bt_batch_confirm_reuses_category_list_pagination_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist category list pagination url")
    ...
    async def fake_page_search(page_url: str) -> list[dict[str, object]]:
        ...
        return [
            ...
            {
                "title": "title-Frieren S01E37",
                "source": "magnet:?xt=urn:btih:3737373737373737373737373737373737373737",
                "infoHash": "3737373737373737373737373737373737373737",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            },
        ]
    ...
    preview_update, preview_reply = _build_update("bt批量 https://nyaa.si/?f=0&c=1_2&p=2 1-2")
    asyncio.run(handle_message(preview_update, context))
    assert preview_reply.await_count == 1
    ...
    sent_text = confirm_reply.await_args.args[0]
    assert sent_text.count("待确认：下载 ⏳") == 2
    assert "片名：title-Frieren S01E36" in sent_text
    assert "片名：title-Frieren S01E37" in sent_text
    assert "确认下载：发送 confirm 1" in sent_text
    assert "确认下载：发送 confirm 2" in sent_text


def test_handle_message_bt_batch_confirm_reuses_category_base_pagination_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist category base pagination url")
    ...
    async def fake_page_search(page_url: str) -> list[dict[str, object]]:
        ...
        return [
            ...
            {
                "title": "title-Frieren S01E39",
                "source": "magnet:?xt=urn:btih:3939393939393939393939393939393939393939",
                "infoHash": "3939393939393939393939393939393939393939",
                "indexerName": "Nyaa",
                "sourceProvider": "nyaa",
            },
        ]
    ...
    preview_update, preview_reply = _build_update("bt批量 https://nyaa.si/?c=1_2&p=2 1-2")
    asyncio.run(handle_message(preview_update, context))
    assert preview_reply.await_count == 1
    ...
    sent_text = confirm_reply.await_args.args[0]
    assert sent_text.count("待确认：下载 ⏳") == 2
    assert "片名：title-Frieren S01E38" in sent_text
    assert "片名：title-Frieren S01E39" in sent_text
    assert "确认下载：发送 confirm 1" in sent_text
    assert "确认下载：发送 confirm 2" in sent_text
...
def test_handle_message_bt_batch_confirm_reuses_search_sort_page_number_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist search sort page number url")
...
def test_handle_message_bt_batch_confirm_reuses_category_sort_page_number_url_preview_candidates() -> None:
    async def unexpected_raw_search(_: str) -> list[dict[str, object]]:
        raise AssertionError("keyword raw search should not be used for allowlist category sort page number url")
