...
def test_record_downloader_approval_logs_missing_result_when_repo_returns_none(capsys) -> None:
    approval_repo = type("ApprovalRepo", (), {"approve_downloader": lambda self, **kwargs: None})()
    service = AddToDownloaderService(
        search_service=SearchMediaService(_fake_search_with_download_url),
        add_torrent_func=AsyncMock(),
        approval_repo=approval_repo,
    ...
    output = capsys.readouterr().out
    assert "[下载确认审批结果缺失]" in output
    assert "downloader approval result missing" in output
    assert "[处理建议]" in output
...
def test_move_completed_approval_identity_logs_persistence_failure(capsys) -> None:
    approval_repo = type(
        "ApprovalRepo",
        (),
    ...
    assert (
        service._move_completed_approval_identity(
            current_task_id="selection:1",
            current_task_hash="candidate:abc123",
            new_task_id="selection:1",
            new_task_hash="hash-42",
        )
        is None
    )
...
def test_mark_completed_job_logs_missing_result(capsys) -> None:
    job_repo = type("JobRepo", (), {"mark_downloader_completed": lambda self, **kwargs: None})()
    service = AddToDownloaderService(search_service=SearchMediaService(_fake_search_with_download_url), add_torrent_func=AsyncMock(), job_repo=job_repo)
    assert (
        service._mark_completed_job(
            job_id="job-1",
    ...
    output = capsys.readouterr().out
    assert "[下载确认任务完结结果缺失]" in output
    assert "downloader completed job result missing" in output
    assert "job_id=job-1" in output
...
def test_confirm_add_by_task_ref_returns_state_unavailable_when_approval_result_is_missing(capsys) -> None:
    job = JobRecord(
        job_id="job-1",
        chat_id=1001,
        user_id=2001,
    ...
    approval_repo = type(
        ...
        (),
        {
            "get_downloader_approval": lambda self, **kwargs: approval_record,
            "is_downloader_pending_expired": lambda self, **kwargs: False,
            "approve_downloader": lambda self, **kwargs: None,
        },
    )()
    add_torrent = AsyncMock(return_value=TransmissionTask(task_id="42", task_hash="hash-42"))
    service = AddToDownloaderService(
...
def test_cancel_pending_add_returns_state_unavailable_when_job_row_missing_with_in_memory_pending(capsys) -> None:
    approval_repo = type(
        "ApprovalRepo",
        (),
        {"cancel_downloader": lambda self, **kwargs: (_ for _ in ()).throw(AssertionError("cancel_downloader should not be called"))},
...
def test_confirm_add_by_task_ref_appends_warning_when_job_completion_result_is_missing(
    capsys: pytest.CaptureFixture[str],
) -> None:
    job = JobRecord(
        job_id="job-1",
    ...
    job_repo = type(
        ...
        (),
        {
            "get_downloader_job_for_chat_ref": lambda self, **kwargs: job,
            "claim_lease": lambda self, **kwargs: True,
            "mark_downloader_completed": lambda self, **kwargs: None,
        },
    )()
    approval_repo = type(
        "ApprovalRepo",
...
def test_confirm_add_by_task_ref_appends_warning_when_approval_identity_move_fails(
    capsys: pytest.CaptureFixture[str],
) -> None:
    job = JobRecord(
        job_id="job-1",
