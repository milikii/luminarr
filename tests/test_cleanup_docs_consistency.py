...
def test_docs_entrypoints_and_snapshot_roles_stay_in_sync() -> None:
    ...
    post_download_auto_import_slimming_log_text = Path("docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md").read_text(
        encoding="utf-8"
    )
    ...
    assert "docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md" in readme_text
    ...
    assert "docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md" in index_text
    ...
    assert "docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md" in next_step_text
    assert "docs/GET_DOWNLOAD_STATUS_SLIMMING_LOG.md" in next_step_text
    assert "docs/CLEANUP_SLIMMING_LOG.md" in status_text
    assert "docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md" in status_text
    assert "docs/GET_DOWNLOAD_STATUS_SLIMMING_LOG.md" in status_text
    assert "post_download_auto_import.py" in post_download_auto_import_slimming_log_text
    assert "自动导入编排层瘦身 / 模块化" in post_download_auto_import_slimming_log_text
...
def test_current_completion_state_docs_do_not_regress() -> None:
    ...
    assert "Jellyfin / Plex 真实联调重评估" in agents_text
    assert "BT 批量任务显式批量确认" in readme_text
    assert "BT 批量任务显式批量确认" in status_text
    assert "BT 批量任务显式批量确认" in next_step_text
    assert "BT 批量任务显式批量确认" in agents_text
    ...
    assert "docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md" in index_text
    assert "docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md" in next_step_text
    assert "docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md" in agents_text
    assert "docs/GET_DOWNLOAD_STATUS_SLIMMING_LOG.md" in readme_text
    assert "docs/GET_DOWNLOAD_STATUS_SLIMMING_LOG.md" in index_text
    assert "docs/GET_DOWNLOAD_STATUS_SLIMMING_LOG.md" in next_step_text
