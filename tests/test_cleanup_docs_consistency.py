...
def test_docs_entrypoints_and_snapshot_roles_stay_in_sync() -> None:
    ...
    assert "独立后台下载完成轮询剩余少量回归与验证收口" in next_step_text
    assert "独立后台下载完成轮询剩余少量回归与验证收口" in status_text
...
def test_download_completion_polling_log_keeps_current_line_detail() -> None:
