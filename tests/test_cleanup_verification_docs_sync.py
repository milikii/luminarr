...
def test_read_windows_env_values_treats_cmd_oserror_as_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _read_windows_env_values()["TELEGRAM_BOT_TOKEN"] == ""
...
def test_run_telegram_bot_api_snapshot_treats_non_auth_http_errors_as_unreachable(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "test-token")
    monkeypatch.setattr("app.maintenance.cleanup_verification_docs.urllib.request.urlopen", lambda url, timeout: (_ for _ in ()).throw(urllib.error.HTTPError(url, 500, "Server Error", hdrs=None, fp=None)))
...
def test_run_local_smoke_evidence_snapshot_keeps_valid_entry_when_log_contains_non_utf8_bytes(tmp_path: Path) -> None:
    docs_dir = tmp_path / "docs"; docs_dir.mkdir(); (docs_dir / "CLEANUP_VERIFICATION_WINDOW.md").write_text("# Cleanup verification window (2026-04-05 to 2026-04-12) (v1)\n\n- 开始日期：2026-04-05\n- 最早可结束日期：2026-04-12\n", encoding="utf-8")
    logs_dir = tmp_path / "logs"; logs_dir.mkdir(); line = build_cleanup_private_chat_smoke_log_line(channel="telegram", query="cleanup inspect abc123", reply_text="已完成检查", chat_id=1, user_id=1, date_text="2026-04-06"); assert line is not None; (logs_dir / "run.log").write_bytes(b"\xff\xfejunk\n" + line.encode("utf-8") + b"\n")
    assert _run_local_smoke_evidence_snapshot(tmp_path) == "found in-window cleanup smoke evidence in repo: telegram; missing channels: personal_wechat,feishu,wecom"
...
def test_sync_documents_keeps_valid_channel_progress_when_log_contains_non_utf8_bytes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
