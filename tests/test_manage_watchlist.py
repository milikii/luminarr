...
def test_parse_watchlist_query_supports_list_add_remove_clear_and_sync() -> None:
    ...
    sync_command = parse_watchlist_query("想看 同步")
    assert sync_command is not None
    assert sync_command.action == "sync"
...
def test_manage_watchlist_add_surfaces_row_corruption(tmp_path: Path, capsys) -> None:
    repo = WatchlistRepo(_make_database(tmp_path))
    ...
    assert reply == WATCHLIST_ADD_FAILED_TEXT
    captured = capsys.readouterr()
    assert "[想看写入命中坏记录]" in captured.out
    assert "[处理建议]" in captured.out
    assert "watchlist_item media kind corrupted after read" in captured.out
...
def test_manage_watchlist_remove_surfaces_row_corruption(tmp_path: Path, capsys) -> None:
    repo = WatchlistRepo(_make_database(tmp_path))
    ...
    assert reply == WATCHLIST_REMOVE_FAILED_TEXT
    captured = capsys.readouterr()
    assert "[想看删除命中坏记录]" in captured.out
    assert "[处理建议]" in captured.out
    assert "watchlist_item media kind corrupted after read" in captured.out
