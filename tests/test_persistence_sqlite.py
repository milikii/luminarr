...
def test_download_monitor_completed_list_rejects_invalid_chat_identity_rows(tmp_path: Path) -> None:
    database = SqliteDatabase(str(tmp_path / "state.sqlite3"))
    database.initialize()
    repo = DownloadMonitorRepo(database)
    ...
    with pytest.raises(
        ...
        match="download monitor completed row chat identity corrupted after read",
    ):
        repo.list_completed_for_auto_import(limit=5)
...
def test_job_repo_raises_when_claim_target_missing_after_update(tmp_path: Path) -> None:
    database = SqliteDatabase(str(tmp_path / "state.sqlite3"))
    database.initialize()
    repo = JobRepo(database)

    with pytest.raises(JobPersistenceError, match="job missing during lease claim"):
        repo.claim_lease(
            job_id="add_to_downloader:hash-1",
            expected_version=1,
            lease_owner="owner-1",
...
def test_approval_repo_rejects_missing_identity_for_requested_lease_lookup(tmp_path: Path) -> None:
    database = SqliteDatabase(str(tmp_path / "state.sqlite3"))
    database.initialize()
    repo = ApprovalRepo(database)

    with pytest.raises(ApprovalPersistenceError, match="approval task identity missing for requested lease query"):
        repo._get_requested_lease_version(
...
def test_approval_repo_cancel_raises_when_row_missing(tmp_path: Path) -> None:
    database = SqliteDatabase(str(tmp_path / "state.sqlite3"))
    database.initialize()
    repo = ApprovalRepo(database)

    with pytest.raises(ApprovalPersistenceError, match="approval_record missing during cancel"):
        repo.cancel_downloader(
            task_id="87",
            task_hash="hash-87",
            task_ref="87",
            expected_lease_version=1,
