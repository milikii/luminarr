...
def test_makefile_verify_mainline_target_points_to_current_focused_regressions() -> None:
    ...
    assert commands[4] == "$(PYTHON) -m pytest -q tests/test_private_chat_status_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k status"
    assert commands[5] == "$(PYTHON) -m pytest -q tests/test_private_chat_import_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_import_query or import_routes_to_import_service or import_formats_import_approval_for_telegram or import_replies_service_not_ready\""
    assert commands[6] == "$(PYTHON) -m pytest -q tests/test_private_chat_watchlist_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_watchlist_query or watchlist_routes_to_watchlist_service or watchlist_series_routes_to_watchlist_service or watchlist_replies_service_not_ready\""
    assert commands[7] == "$(PYTHON) -m pytest -q tests/test_private_chat_bt_subscription_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_bt_subscription_query or bt_subscription_routes_to_service or bt_subscription_run_uses_bound_downloader_context or bt_subscription_replies_service_not_ready or bt_subscription_run_replies_config_missing\""
    assert commands[8] == "$(PYTHON) -m pytest -q tests/test_private_chat_cleanup_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_cleanup_query or cleanup_routes_to_cleanup_service or cleanup_inspect_routes_to_cleanup_service or cleanup_replies_service_not_ready\""
