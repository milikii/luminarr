...
def test_makefile_verify_mainline_target_points_to_current_focused_regressions() -> None:
    ...
    assert commands[9] == "$(PYTHON) -m pytest -q tests/test_private_chat_frustration_runtime.py tests/test_private_chat_runtime.py -k \"handle_frustration_query or cancel or pending_job_lookup_failure\""
    assert commands[10] == "$(PYTHON) -m pytest -q tests/test_private_chat_bt_batch_confirm_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_bt_batch_confirm_query or bt_batch_confirm\""
    assert commands[11] == "$(PYTHON) -m pytest -q tests/test_private_chat_bt_read_only_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_bt_read_only_query or bt_read_only_helper or bt_batch_preview\""
    assert commands[12] == "$(PYTHON) -m pytest -q tests/test_private_chat_search_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_search_query_fallback or routes_search\""
    assert commands[13] == "$(PYTHON) -m pytest -q tests/test_private_chat_status_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k status"
    assert commands[14] == "$(PYTHON) -m pytest -q tests/test_private_chat_import_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_import_query or import_routes_to_import_service or import_formats_import_approval_for_telegram or import_replies_service_not_ready\""
    assert commands[15] == "$(PYTHON) -m pytest -q tests/test_private_chat_watchlist_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_watchlist_query or watchlist_routes_to_watchlist_service or watchlist_series_routes_to_watchlist_service or watchlist_replies_service_not_ready\""
    assert commands[16] == "$(PYTHON) -m pytest -q tests/test_private_chat_bt_subscription_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_bt_subscription_query or bt_subscription_routes_to_service or bt_subscription_run_uses_bound_downloader_context or bt_subscription_replies_service_not_ready or bt_subscription_run_replies_config_missing\""
    assert commands[17] == "$(PYTHON) -m pytest -q tests/test_private_chat_cleanup_runtime.py tests/test_private_chat_runtime.py tests/test_telegram_bot.py -k \"handle_cleanup_query or cleanup_routes_to_cleanup_service or cleanup_inspect_routes_to_cleanup_service or cleanup_replies_service_not_ready\""
