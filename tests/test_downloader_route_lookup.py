...
def test_get_torrent_import_source_with_routing_keeps_client_dir_when_host_dir_unavailable() -> None:
    class FakeJobRepo:
        def get_downloader_job_for_chat_ref(self, *, chat_id: int, task_ref: str):
            assert chat_id == 1001
            assert task_ref == "hash-42"
            return type(
                ...
                (),
                {
                    "payload_json": '{"downloader_name":"missing","download_dir":""}',
                },
            )()
    ...
    import_source = asyncio.run(
        _get_torrent_import_source_with_routing(
            ...
            chat_id=1001,
            job_repo=FakeJobRepo(),
            downloader_instances_by_name={"missing": DownloaderInstanceConfig(
                name="missing",
                downloader_type="transmission",
                base_url="http://127.0.0.1:19092",
                download_dir="",
            )},
            transmission_clients_by_name={"missing": FakeTransmissionClient()},
            qbittorrent_clients_by_name={},
        )
    )
