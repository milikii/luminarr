...
- 当前唯一最小主线：`shared private-chat runtime` 最小抽离。
- 当前目标：把 `handle_private_chat_query_text` 从 `app/bot/telegram_bot.py` 抽到独立 shared runtime 模块，让 Telegram / personal WeChat / Feishu / WeCom 四个渠道都直接调用同一个 channel-agnostic runtime，不再伪造 Telegram `context` 或 `SimpleNamespace`。
- `微信登录`、Telegram 图片/文件回传这类 Telegram-only 能力，要改成显式注入项；shared runtime 不再直接读取 `context.application.bot`。
- 完成标准：
  - `app/bot/private_chat_runtime.py` 不再伪造 Telegram `context`
  - shared runtime 入口不再反向依赖 `app/bot/telegram_bot.py`
...
- 这一步只允许修 shared runtime 回归、渠道胶水回归和显式日志缺口，不做多渠道平台化，也不新增自动 cleanup、批量 cleanup 或删种。
- 当前主线完成后，下一步按顺序推进：
  1. 持久化吞错收口
  2. Feishu 长连接私有 API 风险收口
  3. Feishu 私聊事件解析器去重
  4. 独立后台下载完成轮询剩余少量回归与验证收口
  5. `series / anime` 独立名称解析最小实现（结构化解析 + 小型识别词/替换配置）
  6. `.ass` 字幕支持评估与最小实现
  7. shared private-chat 交付体验收口（图片 / 信息卡片 / 字符排版 / 状态信息清晰化，不做 Web UI）
  8. 最小人类可用入口（quick start / 配置模板 / 首个渠道 10 分钟跑通）
  9. BT 共享确定性评分器
