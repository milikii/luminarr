...
- 想直接把项目跑起来：看 `docs/GETTING_STARTED.md`
  运行时依赖真相也只认这一页：Docker 镜像已内置字幕翻译所需 `ffmpeg` / `ffprobe`，本地 Python 运行时仍需宿主机自备；personal WeChat 登录态、Feishu SDK、WeCom 回调端口与反代要求也都收口在这里
...
- `docs/GETTING_STARTED.md`：安装、启动、最小 smoke，以及运行时依赖真相唯一入口（含 Docker / 本地 Python 差异）
