...
- 如果当前只想确认 WeCom 本地 callback 已就绪，可以先跑 `curl -si http://127.0.0.1:18889/wecom/callback`；这条地址来自当前本地已验证 `.env`，不是 `.env.example` 的默认端口/路径。当前无校验参数时返回 `400 missing echostr` 属于本地入口可达，不等于 WeCom 真实私聊 smoke 已完成。
