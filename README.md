...
- **当前施工状态**：当前进行中的 promoted 主线是 Jellyfin / Plex 真实联调重评估；BT 批量任务显式批量确认已在冷启动审计里确认完成并转入完成态。
...
- **本地回归命令**：当前 Jellyfin / Plex 真实联调重评估入口看 `docs/NEXT_STEP.md`；BT 批量与 PT live seeding 历史蓝图继续看 `docs/BT_BATCH_PLAN.md`、`docs/PT_LIVE_SEEDING_PLAN.md`
- 当前只允许围绕 Jellyfin / Plex readiness 评估施工，不顺手放大到新 Docker 栈、自动探测、全量媒体管理能力或新的 workflow 真相。
