...
- 当前私聊文本入口已补 `cleanup <任务ID或Hash>` / `清理 <任务ID或Hash>` 最小执行闭环
- cleanup 会先基于 `job_event` 中已落地的 `source_path + target_path` 关联校验 `target_path` 仍存在，再只删除单个 downloader/source 侧已导入资产
- 不删 library target、不删 metadata / subtitle sidecar、不顺手删种，也不改现有 `import_to_library` / approval / `confirm` / Telegram / personal WeChat / Feishu / WeCom 既有真相边界

当前 next step：

- downloader/library cleanup inspect baseline（在已落地 cleanup execution + guardrails 之上，只补最小只读预检文本，不扩成自动 cleanup）
...
1. downloader/library cleanup inspect baseline
...
- Telegram richer card/UI polish 当前已先落了搜索结果、下载审批、导入审批三刀；当前主线已经把 cleanup execution 和显式保护栏补上，下一小步先转向 cleanup inspect，只补只读预检，不直接扩成自动清理。
