...
- **当前唯一主线**：独立后台下载完成轮询剩余少量回归与验证收口（把 `telegram_bot.py` 里的待轮询列表、独立轮询 start/stop 和必要的真实链路验证收口成稳定完成态，不改 shared runtime、下载分发、approval 和 SQLite 真相）。
- **详细目标与可测量退出条件**：`docs/NEXT_STEP.md`
- **当前快照**：`docs/STATUS.md`
- **当前主线详细台账**：`docs/DOWNLOAD_COMPLETION_POLLING_LOG.md`
- **上一条主线台账**：`docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`（Feishu 私聊事件解析器去重已在 2026-04-18 达到退出条件 3）
...
- **本地回归命令**：当前主线 focused tests 入口收口在 `docs/DOWNLOAD_COMPLETION_POLLING_LOG.md` 的 2.1–2.2 分组；cleanup 相关回归入口继续见 `docs/GETTING_STARTED.md`。
- 这一步只允许收口后台下载完成轮询的剩余 regression / verification gap，不做 Telegram 渠道层大规模瘦身、scheduler 平台化，也不新增自动 cleanup、批量 cleanup 或删种。
- 当前主线完成后，按 `docs/NEXT_STEP.md` 的 `After this step` 编号顺序推进（`telegram_bot.py` 渠道层瘦身 → 其他编排层瘦身 → `series / anime` 名称解析 → 等等）。
...
- `docs/DOWNLOAD_COMPLETION_POLLING_LOG.md`：当前主线详细闭环
- `docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`：上一条主线详细闭环
