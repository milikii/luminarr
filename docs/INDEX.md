...
| `docs/TELEGRAM_BOT_SLIMMING_LOG.md` | 当前 `telegram_bot.py` 渠道层瘦身 / 模块化主线的详细闭环、focused tests 和风险分组 | 当前主线台账 |
...
1. `docs/NEXT_STEP.md`
2. `docs/STATUS.md`
3. `docs/TELEGRAM_BOT_SLIMMING_LOG.md`（需要看当前主线细节时）
4. `docs/DOWNLOAD_COMPLETION_POLLING_LOG.md`（需要看上一条主线闭环时）
5. `docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`（需要看更早主线闭环时）
6. `docs/FEISHU_LONG_CONNECTION_RISK_LOG.md`（需要看再更早主线闭环时）
7. `docs/PERSISTENCE_CLOSURE_LOG.md`（需要看更早完成主线闭环时）
8. `docs/CLEANUP_VERIFICATION_WINDOW.md`（需要看 cleanup 已完成证据时）
...
- `NEXT_STEP` 只写当前目标，`STATUS` 只写当前快照，当前主线的详细闭环优先收口到对应主线台账（当前为 `docs/TELEGRAM_BOT_SLIMMING_LOG.md`），不要把活动台账全文抄进去。
