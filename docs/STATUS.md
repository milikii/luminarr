...
- 仓库证据快照能力快照：`local_smoke_evidence` 现在在命中窗口期 `[cleanup 私聊 smoke]` 日志时会直接带出 `channel` 列表；当前结果仍是 `no in-window cleanup smoke evidence in repo`，说明仓库里还没有任何窗口内渠道证据可回填。
