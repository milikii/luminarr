...
- 当前这轮只允许先抽 approval 状态存取，不动 `jobs` claim/release/complete、copy-fallback payload 和文件导入执行。
...
- import post-processing focused：`.venv/bin/python -m pytest -q tests/test_import_to_library.py` 为 `142 passed`。
- redesign 基线与 post-processing helper 都已落地：`docs/IMPORT_PIPELINE_REDESIGN.md`、`docs/NEXT_STEP.md`、`docs/IMPORT_TO_LIBRARY_SLIMMING_LOG.md` 已同步当前真相。
...
- 当前最大结构债转移到 services 层三座大山：`app/services/import_to_library.py` `2094` 行 / `app/services/add_to_downloader.py` `1669` 行 / `app/services/search_media.py` `1018` 行，合计 `4781` 行，占全仓 `25663` 行的 `18.6%`。
- 风险消除路径：`post_import_side_effects` 已经收出主文件，当前最该继续收口的是 approval lease/version 状态存取；等 approval helper 稳住后，再碰 `jobs` 状态迁移。
