...
- 当前又从 cleanup backlog 再收掉一组闭环：`job_event` 追加护栏、inspection 计算、follow-up / blocked detail 组装，以及删除执行与成功/失败消息组装都已下沉到 support helper；当前 `cleanup_downloaded_source.py` 已降到 `650` 行。
- 当前又从 cleanup backlog 再收掉几小格：命令解析、inspect 渲染、blocked outcome 编排与 PT seed guard 评估也已继续下沉到 support helper；当前 `cleanup_downloaded_source.py` 已降到 `518` 行。
...
- 2026-04-25 cleanup 结构收口 follow-up：`.venv/bin/python -m pyflakes app/services/cleanup_downloaded_source.py app/services/cleanup_event_support.py app/services/cleanup_inspection_support.py app/services/cleanup_follow_up_support.py app/services/cleanup_execution_support.py` 通过；`.venv/bin/python -m pytest -q tests/test_cleanup_downloaded_source.py` 为 `49 passed`
- 2026-04-25 cleanup 结构收口 follow-up 2：`.venv/bin/python -m pyflakes app/services/cleanup_downloaded_source.py app/services/cleanup_query_support.py app/services/cleanup_event_support.py app/services/cleanup_inspection_support.py app/services/cleanup_follow_up_support.py app/services/cleanup_execution_support.py app/services/cleanup_inspect_render_support.py app/services/cleanup_blocked_support.py app/services/cleanup_seed_guard_support.py` 通过；`.venv/bin/python -m pytest -q tests/test_cleanup_downloaded_source.py` 为 `49 passed`
