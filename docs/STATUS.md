...
- shared runtime / channel 解耦已收掉 52 条最小直连；本轮最新闭环是把 `private_chat_runtime.py` 开头的 `tg / execution_gate / traced reply` 装配抽到 `_prepare_private_chat_runtime_bootstrap()`。
...
- 当前主线 focused 验证入口：绿灯；本轮 private-chat runtime bootstrap focused 为 `4 passed, 52 deselected`。
...
- 当前闭环 focused：`tests/test_private_chat_runtime.py -k "routes_search or writes_trace_log or personal_wechat_login"` 为 `4 passed, 52 deselected`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债仍在 `app/bot/private_chat_runtime.py`：文件当前为 `363` 行；`app/bot/telegram_bot.py` 已降到 `256` 行，主文件里的纯 wrapper 已基本清空。
- 当前更小也更直接的下一块热点，是 `private_chat_runtime.py` 里 BT 批量确认 / BT TMDB / raw BT destination / digit-selection 仍各自内联 downloader execution lambda；这块比直接切更大的 service 文件更适合作为下一条 shared runtime / channel 解耦闭环。
