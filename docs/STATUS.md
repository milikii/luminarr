...
## Current mainline
- **质量硬化** 当前保持完成态，不回退。
- `config.py` 重复解析逻辑已收口：`RAW_BT_DESTINATIONS` / `ADULT_ARCHIVE_DESTINATIONS` / `DOWNLOADER_INSTANCES` 共用解析 helper，base URL 也统一走 shared normalization helper。
...
## Latest verification
- `tests/test_config.py tests/test_downloader_route_lookup.py tests/test_main.py`：`66 passed, 0 skipped`
- `make quality`：通过
