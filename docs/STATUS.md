...
- 2026-04-15 代码审查确认：`cleanup_smoke_logging` 已支持显式 `log_path` 传参、测试侧也已有 reset helper 复位默认路径；追加链已经不再读取模块级 `_cleanup_private_chat_smoke_log_path`，configure helper 成功态和失败态也都不再写这份状态；当前剩余风险收口为“只剩 reset helper 自身仍保留这份模块级状态”。
