...
- 当前又从 cleanup backlog 再收掉几小格：命令解析、inspection、inspect 渲染、blocked outcome、删除执行、PT seed guard、相关日志与 cleanup 主流程编排都已下沉到 support helper；当前 `cleanup_downloaded_source.py` 已降到 `340` 行。
- 当前又从 cleanup correlation backlog 再收掉几小格：关联事件读取护栏、路径缺失判定 / 结果构造，以及任务身份解析也已下沉到 helper；当前 `cleanup_correlation_lookup.py` 已降到 `232` 行。
...
- 2026-04-25 cleanup 结构收口 follow-up：cleanup 相关 support helper 的 `pyflakes` 与 `tests/test_cleanup_downloaded_source.py` 均通过；focused 结果为 `49 passed`
- 2026-04-25 cleanup 结构收口 follow-up 2：`cleanup_flow_support.py` 已纳入 cleanup helper 边界；对应 `pyflakes` 与 `tests/test_cleanup_downloaded_source.py` 仍为通过，focused 结果保持 `49 passed`
- 2026-04-25 cleanup correlation 结构收口 follow-up：新增 `cleanup_correlation_event_support.py`、`cleanup_correlation_result_support.py` 与 `cleanup_task_identity_support.py` 后，`pyflakes` 与 `tests/test_cleanup_downloaded_source.py` 仍通过；focused 结果保持 `49 passed`
