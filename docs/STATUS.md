...
- 本轮继续收口导入后处理和 BT 跟进边界：`import_post_processing` 的 metadata/subtitle/refresh 只吞掉明确 `RuntimeError`，`subtitle_translation_support` 的模型翻译失败只吞掉明确 `RuntimeError`，`refresh_media_server` 只吞掉明确 `httpx.HTTPError`，`search_recovery_runtime` 只对明确 `RuntimeError` 进入恢复。
- 本轮同时补了 BT 只读探索、raw BT 目标目录、BT TMDB 关联的 focused regression，把相关网络失败模拟统一收成真实 `httpx.ConnectError`，并补了“非预期 `ValueError` 继续上抛”的守卫测试。
...
## Current health
- 默认分支质量 gate 通过；本轮 focused tests 覆盖 import post-processing、refresh media server、subtitle translator、BT 只读 / raw BT / BT TMDB follow-up 与 search recovery。
...
## Latest verification
- `tests/test_import_to_library.py tests/test_private_chat_bt_read_only_runtime.py tests/test_private_chat_bt_tmdb_runtime.py tests/test_private_chat_raw_bt_destination_runtime.py tests/test_refresh_media_server.py tests/test_search_recovery_runtime.py tests/test_subtitle_translator.py` 通过（`217 passed`）。
