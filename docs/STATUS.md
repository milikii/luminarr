...
- 当前又从刮削后续 backlog 收掉一小格：`subtitle_translator` 的 chunk 翻译编排与异常校验已下沉到 `app/services/subtitle_translation_support.py`，`app/services/subtitle_translator.py` 已回落到 `545` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `15 passed`
