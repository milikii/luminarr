...
- 2026-04-15 代码审查确认：`search_media` 里搜索候选持久化失败现在已补红色中文 `[搜索候选持久化失败]` 和 `[处理建议]`，不再静默吞掉 `candidate_repo.save_candidates()` 异常；当前剩余风险收口为“候选读取、澄清态 upsert/clear/load 以及其他持久化路径仍有 `except Exception: pass/return None` 会把‘真没数据’和‘SQLite/配置异常’混写”。
