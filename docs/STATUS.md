...
- 当前没有进行中的 promoted 主线；上一条完成主线是 Jellyfin / Plex。后续若要继续做其他事项，先更新 `docs/NEXT_STEP.md` 完成切线，再开工。
...
- 当前没有新 promoted 主线；如果不先在 `docs/NEXT_STEP.md` 把下一条事项提升成主线，就不应继续改业务边界。
- Jellyfin / Plex 当前只完成 provider 选择和最小 refresh baseline，不在这一步扩成自动探测或更完整的媒体管理能力。
- cleanup 已完成，但 `pt_min_seed_hours` 保护仍是已知边界，未在当前完成态里补成新主线。
- 字幕翻译当前仍只处理 `.srt`；`.ass` 继续留作后续能力缺口，不影响当前完成态。

## Latest verification

- 窗口活性快照：`jellyfin / plex` 主线已完成，当前无新 promoted 主线
- 当前状态快照：等待下一条 promoted 主线
- 当前结论快照：当前文档与近 20 条提交一致表达“无进行中的 promoted 主线”；`docs/NEXT_STEP.md` 现行 `Done when` 已收口，旧版 3 条退出条件只保留在历史台账语境，不再作为当前执行真相。
...
- focused cleanup tests：2026-04-14，`526 passed, 93 deselected`（`.venv/bin/python -m pytest -q tests/test_cleanup_cross_channel_smoke.py tests/test_cleanup_downloaded_source.py tests/test_private_chat_runtime.py tests/test_personal_wechat_text.py tests/test_feishu_adapter.py tests/test_wecom_adapter.py tests/test_telegram_bot.py -k cleanup`）
- cleanup verification docs gate：2026-04-14，`384 passed`（`.venv/bin/python -m pytest -q tests/test_cleanup_docs_consistency.py tests/test_cleanup_verification_window_doc.py tests/test_cleanup_cross_channel_smoke.py`）
- focused config truth tests：2026-04-19，`4 passed, 21 deselected`（`.venv/bin/python -m pytest -q tests/test_config.py -k "requires_token or requires_transmission_base_url or defaults_role_binding_to_first_instance or reads_tmdb_settings"`）
