...
- 本轮继续统一 Telegram sidecar 日志出口：`telegram_sidecar_runtime` 的 BT 订阅“后台扫描失败”与“后台通知失败”日志已切到 shared operational logging。
- 本轮补了 focused regression，守住日志标题、detail 和 fix hint 不漂移。
...
## Current health
- 默认分支质量 gate 通过；本轮 focused tests 覆盖 BT 订阅 sidecar 扫描失败与通知失败日志出口。
- 本轮未触发真实 downloader / refresh 协议行为变化；改动为 Telegram sidecar 失败日志出口统一，并由 focused tests 与 mainline gate 覆盖。
...
## Latest verification
- `tests/test_telegram_bot.py -k "test_log_bt_subscription_scheduler_loop_error_uses_shared_log or test_log_bt_subscription_scheduler_send_error_uses_shared_log"` 通过（`2 passed`，`194 deselected`）。
