...
- 当前又从后续结构 backlog 再收掉一组 cleanup 小闭环：`job_event` 追加护栏、inspection 计算、follow-up / blocked detail 组装，以及删除执行与成功/失败消息组装都已下沉到 support helper；当前 `cleanup_downloaded_source.py` 已降到 `650` 行。
