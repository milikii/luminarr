...
- 当前阶段已切到 **质量硬化**。
- 默认分支已在本轮再次复验全量回归绿灯：`.venv/bin/python -m pytest -q` 为 `1625 passed, 2 skipped`。
- shared runtime / channel 解耦已先收掉 5 条最小直连：`raw_bt` 目录问询改为复用 `app/bot/raw_bt_destination_runtime.py`，BT TMDB 关联问询改为复用 `app/bot/bt_tmdb_association_runtime.py`，下载器角色绑定解析改为复用 `app/bot/downloader_execution_runtime.py`，execution gate / sync policy helper 改为复用 `app/bot/execution_runtime.py`，frustration / BT intent / BT choice 文本解析 helper 改为复用 `app/bot/query_text_runtime.py`。
...
- 全量回归：绿灯；最近一次 `.venv/bin/python -m pytest -q` 为 `1625 passed, 2 skipped`。
...
- 全量回归：`.venv/bin/python -m pytest -q` 为 `1625 passed, 2 skipped`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债不再是测试红灯，而是 shared runtime 仍直接复用 `telegram_bot.py` 里的 search reactive recovery helper。
- `app/bot/private_chat_runtime.py`、`app/bot/telegram_bot.py` 仍是热点大文件；query text helper 已先收口，下一段更小也更中性的边界是 `_search_with_reactive_recovery`、`_build_recovery_context`、`_is_llm_physical_failure` 这一组搜索恢复 helper。
