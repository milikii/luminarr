...
- 2026-04-15 代码审查确认：`cleanup_smoke_logging` 已支持显式 `log_path` 传参、目录创建失败 fail-closed 返回 `None`，测试侧也已有 reset helper 复位默认路径；追加链已经不再读取模块级 `_cleanup_private_chat_smoke_log_path`，当前剩余风险收口为“configure/reset helper 自身仍保留这份模块级状态”。
