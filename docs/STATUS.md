...
- 当前三条 BT 路径仍各自有一套简化选源逻辑：`pure_bt.py`、`manage_bt_subscription.py` 和媒体型 BT 候选展示还没共用同一套确定性评分 helper。
- 当前主线第一闭环还没落地：`app/services/bt_candidate_scorer.py`、`tests/test_bt_candidate_scorer.py` 和可选规则文件都还不存在。
