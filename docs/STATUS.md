...
- shared runtime / channel 解耦已收掉 53 条最小直连；本轮最新闭环是把 `private_chat_runtime.py` 里 4 处 downloader execution lambda 收成局部 shared resolver。
...
- 当前主线 focused 验证入口：绿灯；本轮 private-chat downloader execution focused 为 `2 passed, 54 deselected`。
...
- 当前闭环 focused：`tests/test_private_chat_runtime.py -k "routes_bt_batch_confirm_without_telegram_update or routes_add_pending_with_channel_delivery_renderer"` 为 `2 passed, 54 deselected`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债仍在 `app/bot/private_chat_runtime.py`：文件当前为 `359` 行；`app/bot/telegram_bot.py` 已降到 `256` 行，主文件里的纯 wrapper 已基本清空。
- 当前更小也更直接的下一块热点，是 `private_chat_runtime.py` 中段 status / watchlist / BT subscription / import / cleanup 这段 execution-gated shared route 仍直接堆在主函数里；这块比直接切更大的 service 文件更适合作为下一条 shared runtime / channel 解耦闭环。
