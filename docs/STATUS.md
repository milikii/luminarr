...
- 2026-04-19 已执行一次真实 Jellyfin refresh 失败探针；当前主机只发现 Emby 在监听，`http://127.0.0.1:8096/Library/Refresh` 返回 `All connection attempts failed`，失败点已可直接定位到 target 和 request_url。
