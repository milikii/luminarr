...
- 当前阶段已切到 **质量硬化**。
- 默认分支已在本轮再次复验全量回归绿灯：`.venv/bin/python -m pytest -q` 为 `1650 passed, 2 skipped`。
- shared runtime / channel 解耦已收掉 25 条最小直连；本轮最新闭环是把 `private_chat_runtime.py` 里的 personal WeChat 登录路由抽到 `app/bot/private_chat_login_runtime.py`，并把 login focused tests 补进 `verify-mainline`。
...
- 当前主线 focused 验证入口：绿灯；本次 `verify-mainline` 为 `25 passed, 17 deselected` + `14 passed, 192 deselected` + `3 passed, 55 deselected` + `5 passed, 247 deselected`。
- 全量回归：绿灯；最近一次 `.venv/bin/python -m pytest -q` 为 `1650 passed, 2 skipped`。
...
- 全量回归：`.venv/bin/python -m pytest -q` 为 `1650 passed, 2 skipped`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债仍在 `app/bot/private_chat_runtime.py`：文件已降到 `1204` 行，但仍承载 BT follow-up、状态查询路由和搜索兜底等多段共享逻辑；`app/bot/telegram_bot.py` 保持 `661` 行。
- 当前更小也更直接的下一块热点，是 `private_chat_runtime.py` 里的状态查询路由仍直接读取 `GetDownloadStatusService` 并触发共享 follow-up；这块比继续深入 BT follow-up 更贴近 shared runtime / service 解耦目标。
