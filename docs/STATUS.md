...
- 2026-04-15 代码审查确认：`cleanup_smoke_logging` 已支持显式 `log_path` 传参；追加链已经不再读取模块级 `_cleanup_private_chat_smoke_log_path`，configure helper 成功态和失败态、以及 reset helper 都不再写这份状态；死变量也已删掉；当前剩余风险收口为“只剩兼容 API 壳子 `reset_cleanup_private_chat_smoke_log_file()`”。
