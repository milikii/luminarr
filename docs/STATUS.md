...
- 当前阶段已切到 **质量硬化**。
- 默认分支已在本轮再次复验全量回归绿灯：`.venv/bin/python -m pytest -q` 为 `1645 passed, 2 skipped`。
- shared runtime / channel 解耦已收掉 23 条最小直连；本轮最新闭环是把 `private_chat_runtime.py` 里的 digit-selection 路由抽到 `app/bot/private_chat_selection_runtime.py`，澄清态判断、下载器解析和 add 调度现在有独立 focused tests，不改审批语义。
...
- 全量回归：`.venv/bin/python -m pytest -q` 为 `1645 passed, 2 skipped`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债仍在 `app/bot/private_chat_runtime.py`：文件已降到 `1280` 行，但仍承载 BT follow-up、trace 包装和搜索兜底等多段共享逻辑；`app/bot/telegram_bot.py` 保持 `661` 行。
- 当前更小也更直接的下一块热点，是 `private_chat_runtime.py` 的 trace 包装和入站日志仍挤在主调度函数前半段；这块比继续深入 BT follow-up 更容易切成新的最小闭环。
