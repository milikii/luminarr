...
- shared runtime / channel 解耦已收掉 13 条最小直连；本轮最新闭环是把 `private_chat_runtime.py` 里的 BT 只读探索 / cleanup 路由各自抽成执行 helper，路由主干不再内联 service 获取、失败日志和 reply / smoke 编排。
...
- 下一段更小也更中性的结构债，是把 `private_chat_runtime.py` 里的 BT 批量确认路由分支继续拆成共享小 helper，减少同一入口里编号校验、service 获取、下载器绑定解析和执行编排的混排。
