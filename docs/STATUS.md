...
- 当前又从刮削后续 backlog 收掉一小格：`ffprobe` 的结果解析与错误映射也已下沉到 `subtitle_translation_support.py`，并补了“`ffprobe` 输出不是有效 JSON 时返回明确失败结果”的 focused 护栏。当前 `subtitle_translator.py` 为 `505` 行，`_probe_embedded_subtitles` 已收短到 `40` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `35 passed`
