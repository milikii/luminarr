...
- 当前下载链 focused 验证：`tests/test_add_to_downloader.py` 为 `111 passed`。
...
- add_to_downloader focused：`.venv/bin/python -m pytest -q tests/test_add_to_downloader.py` 为 `111 passed`。
...
- 当前最大结构债仍在 services 层两座大山：`app/services/add_to_downloader.py` `937` 行 / `app/services/import_to_library.py` `1392` 行；`app/services/search_media.py` 已降到 `460` 行。
- 风险消除路径：`search_media.py` 已先达标；`add_to_downloader.py` 的 jobs 状态机、approval / lease 查询、confirm context / expiry 和 pending approval persistence 都已拆出，当前最厚也最危险的是 finalization / approval identity 尾部编排和 `import_to_library.py` 的剩余执行编排壳。
