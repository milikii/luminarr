...
- **质量硬化** 当前保持完成态，不回退。
- 当前默认分支主线的 **重复 trace logger 收口 / 质量硬化**、**`_COMPAT_REEXPORTS` 死代码清理 / 质量硬化** 都已完成；`app/main.py` 与 `app/bot/telegram_bot.py` 的 `_COMPAT_REEXPORTS` 已移除。若继续，下一条更保守候选再评估 `config.py` 重复解析逻辑。
...
- 当前归档迁移、cleanup 收口、trace logger 收口和 `_COMPAT_REEXPORTS` 清理都已落地；下一条最小风险应继续挑窄而重复的结构债，而不是回头再造 `cleanup_*_support.py`、workflow trace 壳或兼容 tuple。

## Later candidate line

- 当前 cleanup 支持文件收口、重复 trace logger 收口和 `_COMPAT_REEXPORTS` 清理都已完成；下一条更保守候选可切去 `config.py` 重复解析逻辑，再之后才是用户可感知改进。
...
- 当前 biggest risk 已不再是 cleanup 支持文件、trace logger 重复壳或 `_COMPAT_REEXPORTS`；下一轮如果继续减法，应改挑新的窄结构债，例如 `config.py` 的重复解析逻辑。
...
这轮 `_COMPAT_REEXPORTS` 清理也已完成。若继续，默认不要回头重建 cleanup 薄壳、workflow trace 壳或兼容 tuple；优先再评估 `config.py` 重复解析逻辑。
```
