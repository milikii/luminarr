...
- `app/downloader_route_lookup.py` 当前 `445` 行；重复的 route lookup 日志与 dispatch 日志已开始收成共享打印器，import/status/remove 三条路由的“先拿 route 再拿 client”前半段已抽到共享 helper，client 选择与 payload key 读取也已改成共享壳，不改错误文本或路由语义。
