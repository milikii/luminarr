...
- `docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`：当前“Feishu 私聊事件解析器去重”详细台账
...
当前快照按主题归纳；当前主线具体路径、focused tests 和风险分组统一只看 `docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`，上一条主线详细闭环继续只看 `docs/FEISHU_LONG_CONNECTION_RISK_LOG.md`，更早主线继续只看 `docs/PERSISTENCE_CLOSURE_LOG.md`，状态页不逐天或逐字段追加条目。

- 当前唯一主线已切到“Feishu 私聊事件解析器去重”；Feishu 长连接私有 API 风险收口已在 2026-04-18 满足 `Done when` 第 1 条，持久化吞错收口、cleanup 四渠道验证窗口与 shared private-chat runtime 最小抽离保持完成态。
...
- 当前新主线只收口 Feishu webhook payload 与 SDK event 的重复解析分支，不改 shared runtime、回包协议和现有渠道边界；详细闭环改看 `docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`。
...
- 当前主风险是 `app/bot/feishu_adapter.py` 仍并排维护 webhook payload 与 SDK event 两套 Feishu 私聊文本字段提取分支；下一步只收口这层解析重复。
- 当前必须继续守住已经落下来的 fail-closed 方向，不能在做 Feishu 解析器去重时回退下载 / 导入 / 搜索 / BT 待答边界。
...
- 当前主线切换审计：2026-04-18，`passed`（`.venv/bin/python -m pytest -q tests/test_feishu_long_connection.py`；`rg -n "lark_ws_client_module\\.loop|_disconnect|_auto_reconnect|_cache" app/bot/feishu_long_connection.py` 命中 `0`）
- 上一条主线切换审计：2026-04-18，`passed`（`bash -lc "git grep -n 'except Exception:\\s*\\(pass\\|return None\\)' app/services app/db app/bot | wc -l"`，命中 `0`）
- 上一条主线 focused verification：2026-04-18，`2 passed, 131 deselected`（`.venv/bin/python -m pytest -q tests/test_import_to_library.py -k "context_lookup or context_row_corruption"`）
- 上一条主线 focused verification：2026-04-18，`3 passed, 129 deselected`（`.venv/bin/python -m pytest -q tests/test_import_to_library.py -k "naming_truth"`）
- 上一条主线 focused verification：2026-04-18，`8 passed, 123 deselected`（`.venv/bin/python -m pytest -q tests/test_import_to_library.py -k "raw_bt"`）
- 上一条主线 focused verification：2026-04-18，`6 passed, 147 deselected`（`.venv/bin/python -m pytest -q tests/test_telegram_bot.py -k "dedup_result_missing or dedup_persist_fails or update_id_invalid or callback_id_missing"`）
- 上一条主线 focused verification：2026-04-18，`3 passed, 148 deselected`（`.venv/bin/python -m pytest -q tests/test_telegram_bot.py -k "pending_list"`）
