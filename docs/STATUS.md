...
- `docs/FEISHU_LONG_CONNECTION_RISK_LOG.md`：当前“Feishu 长连接私有 API 风险收口”详细台账
- `docs/PERSISTENCE_CLOSURE_LOG.md`：已完成的“持久化吞错收口”详细台账
- `docs/CLEANUP_VERIFICATION_WINDOW.md`：cleanup 已完成窗口的详细证据

## What is implemented now

当前快照按主题归纳；当前主线具体路径、focused tests 和风险分组统一只看 `docs/FEISHU_LONG_CONNECTION_RISK_LOG.md`，上一条主线详细闭环继续只看 `docs/PERSISTENCE_CLOSURE_LOG.md`，状态页不逐天或逐字段追加条目。

- 当前唯一主线已切到“Feishu 长连接私有 API 风险收口”；持久化吞错收口已在 2026-04-18 冷启动审计中满足 `Done when` 第 3 条，cleanup 四渠道验证窗口与 shared private-chat runtime 最小抽离保持完成态。
...
- Feishu 长连接当前已可把 SDK 私聊事件转进 shared runtime，但启停仍直接依赖 SDK 私有属性 / 私有模块状态；新主线只收口这一层风险，不改 webhook 路径和 shared runtime 业务边界。
- cleanup 完成态、四渠道真实 smoke 证据和窗口 gate 继续只维护在 `docs/CLEANUP_VERIFICATION_WINDOW.md`；当前主线详细闭环改看 `docs/FEISHU_LONG_CONNECTION_RISK_LOG.md`。
...
- 主线切换审计：2026-04-18，`passed`（`bash -lc "git grep -n 'except Exception:\\s*\\(pass\\|return None\\)' app/services app/db app/bot | wc -l"`，命中 `0`）
