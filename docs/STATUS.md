...
- 当前又从刮削后续 backlog 收掉一小格：`subtitle_translator` 现在会对目录型目标按单个视频逐集判断外挂/内嵌字幕，不再因为某一集已有中文字幕或外挂字幕而把整季目录整段跳过；同一目录里“某些集有外挂字幕、另一些集只能走内嵌提取”的混合场景也已可逐集翻译。相关编排已继续下沉到 `app/services/subtitle_translation_support.py`；当前 `subtitle_translator.py` 为 `517` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `21 passed`
