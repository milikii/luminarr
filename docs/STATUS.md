...
- 2026-04-15 代码审查确认：`cleanup_smoke_logging` 已支持显式 `log_path` 传参；追加链已经不再读取模块级 `_cleanup_private_chat_smoke_log_path`，configure helper 成功态和失败态、以及 reset helper 都不再写这份状态；当前剩余风险收口为“模块里还留着未删掉的死变量和兼容 API 壳子”。
