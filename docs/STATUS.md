...
- shared runtime / channel 解耦已收掉 12 条最小直连；当前最新三条是 `raw_bt_destination_runtime.py` 的 pending / flow / pure-BT logging 收口，以及 `bt_tmdb_association_runtime.py` 的 pending / flow / lookup / logging 收口。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债不再是测试红灯，而是 shared runtime 仍直接复用 `telegram_bot.py` 里的 BT 只读探索 / cleanup 日志 helper。
- `app/bot/private_chat_runtime.py`、`app/bot/telegram_bot.py` 仍是热点大文件；BT follow-up lookup / logging helper 已收口后，下一段更小也更中性的边界是 `_log_bt_read_only_helper_error`、`_log_cleanup_service_not_ready` 这一组只读失败 / cleanup 未就绪日志 helper。
