...
- BT 当前已经有关键词只读搜索、批量预览和显式批量确认；allowlist 页面 URL 的只读预览、聊天缓存和 Telegram 路由证明也已落地，当前还缺“页面预览候选复用到 `bt批量确认`”的更直接 focused proof。
- 当前最小风险是保持页面 URL 只读预览和中文拒绝不回退，再决定要不要补更省输入的页码语法糖。
...
- BT page preview focused tests：2026-04-19，`10 passed, 47 deselected`（`.venv/bin/python -m pytest -q tests/test_bt_sources.py tests/test_search_media.py -k "page or bt_batch"`）
- BT page preview route/cache focused tests：2026-04-19，`4 passed, 203 deselected`（`.venv/bin/python -m pytest -q tests/test_search_media.py tests/test_telegram_bot.py -k "bt_batch and page"`）
...
- BT batch confirm focused tests：2026-04-19，`11 passed, 312 deselected`（`.venv/bin/python -m pytest -q tests/test_add_to_downloader.py tests/test_pure_bt.py tests/test_search_media.py tests/test_telegram_bot.py -k bt_batch`）
