...
- `docs/IMPORT_TO_LIBRARY_SLIMMING_LOG.md`：当前“`import_to_library.py` 导入编排层瘦身 / 模块化”详细台账
- `docs/TELEGRAM_BOT_SLIMMING_LOG.md`：已完成的“`telegram_bot.py` 渠道层瘦身 / 模块化”详细台账
...
当前快照按主题归纳；当前主线具体路径、focused tests 和风险分组统一只看 `docs/IMPORT_TO_LIBRARY_SLIMMING_LOG.md`，上一条主线详细闭环继续只看 `docs/TELEGRAM_BOT_SLIMMING_LOG.md`，更早主线继续只看 `docs/DOWNLOAD_COMPLETION_POLLING_LOG.md`、`docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`、`docs/FEISHU_LONG_CONNECTION_RISK_LOG.md` 和 `docs/PERSISTENCE_CLOSURE_LOG.md`，状态页不逐天或逐字段追加条目。

- 当前唯一主线已切到“`import_to_library.py` 导入编排层瘦身 / 模块化”；`telegram_bot.py` 渠道层瘦身主线已在 2026-04-19 满足 `Done when` 第 1 条，独立后台下载完成轮询、Feishu 私聊事件解析器去重、Feishu 长连接私有 API 风险收口、持久化吞错收口、cleanup 四渠道验证窗口与 shared private-chat runtime 最小抽离保持完成态。
...
- 当前新主线只收 `import_to_library.py` 里的一个连贯切片，优先处理导入前上下文重建 / raw_bt 判定，或执行模式 / copy-fallback / 文件系统导入执行 / metadata / subtitle / refresh 收尾中的一组，不改 approval、`jobs`、`job_event`、导入成功真相和现有副作用边界；详细闭环改看 `docs/IMPORT_TO_LIBRARY_SLIMMING_LOG.md`。
...
- 当前主风险是 `app/services/import_to_library.py` 仍把 confirm 上下文重建、raw_bt 判定、执行模式 / copy-fallback、文件系统导入执行和 metadata / subtitle / refresh 收尾揉在同一文件里；下一步只能拆一个连贯切片，不能顺手大改。
- 当前必须继续守住已经落下来的 fail-closed 方向，不能在做 `import_to_library.py` 瘦身时回退 approval、`jobs`、`job_event`、导入成功真相和现有后置副作用边界。
...
- 窗口活性快照：已满足退出条件
- 当前状态快照：进行中
- 当前结论快照：`telegram_bot.py` 渠道层瘦身主线已在 2026-04-19 通过 `handle_callback_query/build_application` focused tests 满足退出条件 1；当前主线已切到 `import_to_library.py` 导入编排层瘦身 / 模块化。
...
- 当前主线 baseline verification：2026-04-18，`2 passed, 131 deselected`（`.venv/bin/python -m pytest -q tests/test_import_to_library.py -k "context_lookup or context_row_corruption"`）
- 当前主线 baseline verification：2026-04-18，`3 passed, 129 deselected`（`.venv/bin/python -m pytest -q tests/test_import_to_library.py -k "naming_truth"`）
- 当前主线 baseline verification：2026-04-18，`8 passed, 123 deselected`（`.venv/bin/python -m pytest -q tests/test_import_to_library.py -k "raw_bt"`）
