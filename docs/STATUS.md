...
- 上一条 downloader 主线 focused 验证：`tests/test_add_to_downloader.py -k "rebuild_confirm_context or claim_pending_job or confirm_add_by_task_ref or register_download_monitor or record_event"` 为 `40 passed, 71 deselected`；`tests/test_add_to_downloader.py` 为 `111 passed`。
...
- 当前最大结构债仍在 services 层三座大山：`app/services/add_to_downloader.py` `1549` 行 / `app/services/import_to_library.py` `1392` 行 / `app/services/search_media.py` `1018` 行。
- 风险消除路径：`add_to_downloader.py` 的 pending prepare 与 confirm follow-up 两段已经分开，当前只剩 `cancel_pending_add()` 这一块还值得做一次收益重评；如果它没有清晰 helper，就停止继续在这座山上微切分并切去 `search_media.py`。
