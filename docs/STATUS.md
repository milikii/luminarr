...
- 当前 promoted 主线已切到 **BT 方向剩余用户价值重评估**：2026-04-20 冷启动审计确认上一条 **BT 批量确认多条连续真实 dispatch smoke 值得性评估** 已按“只剩重复取证”收口；同一轮健康探针再次观测到 `PT Transmission(19091)` 可达、`Emby(18096)` 可达，但 `BT Transmission(19092)` 仍不可达。
...
- 当前最小风险是：当前 shell 里 `Emby(18096)` 可达，但 BT 下载器测试实例 `19092` 仍不可达，且本地 `.env` 没有现成的 `BT_DOWNLOADER / DOWNLOADER_INSTANCES` 角色绑定；在这个前提没恢复前，不值得继续留在 BT proof 家族里拆更小 smoke。
...
- 窗口活性快照：当前主线为 BT 方向剩余用户价值重评估
- 当前状态快照：Plex 这条线已按“当前主机无可达实例”收口；当前真实 refresh 测试栈仍以 Emby 为正式入口。
- 当前结论快照：近 20 条提交与当前完成态记录一致；`PERSISTENCE_CLOSURE_LOG.md` 2.1~2.4 已覆盖 10 个指定 repo 的全部对外方法，`git grep 'except Exception:\\s*\\(pass\\|return None\\)' app/services app/db app/bot` 命中仍为 `0`；BT 页面 URL proof、单条 BT 真实 dispatch、BT 批量确认真实 dispatch 与“多条连续真实 dispatch 值得性评估”都已收口，当前 promoted 主线切到“BT 方向剩余用户价值重评估”。
