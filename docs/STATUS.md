...
- 上一条 import 主线 focused 验证：`tests/test_import_to_library.py -k "cancel_pending_import or expired_pending_confirm"` 为 `15 passed, 127 deselected`；`tests/test_import_to_library.py` 为 `142 passed`。
...
- 当前最大结构债回到 services 层三座大山：`app/services/add_to_downloader.py` `1669` 行 / `app/services/import_to_library.py` `1392` 行 / `app/services/search_media.py` `1018` 行。
- 风险消除路径：`import_to_library.py` 的 context / approval / jobs / file-transfer / cancel 五段都已收口，继续在这座山上微切分收益已明显递减；当前应该转去 `add_to_downloader.py` 的 confirm 执行 / 下载监控 / 事件落盘厚块。
