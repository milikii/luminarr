...
- 当前这一轮又前推一格：`AMZN / DSNP / NF / ATVP / HMAX / iTunes` 这类 BT 结果来源标签现在也会按标题噪音处理；同片同分辨率的 WEB-DL 不会再只因来源标签不同就在第一屏连着出现两条。
- 当前这一轮再补一格：`4K` 与 `2160p` 这类等价分辨率写法现在也会按同一档 UHD 结果处理；同片 UHD 资源不会再只因写法不同就在第一屏重复占位。
...
- movie-first 去重与 BT 标题相关性现在也会忽略常见流媒体来源标签；`AMZN`、`DSNP` 这类来源差异不会再把同片 WEB-DL 变成两条前台候选。
- movie-first 去重现在也会把 `4K` 解析成 `2160p`；同片 UHD 资源不会再因为 `4K / 2160p` 写法不同保留成两条候选。
- 搜索链结构继续收口：query parser、标题噪音规则、TMDB 高置信判断、等价 query 去重、BT 排序器和 fallback token 现在都围绕共享标题归一层实现；当前 BT 标题噪音 stopwords 也已回收到共享层，后续补 scene/source/tag 噪音词不需要再在两处各改一份。
...
如果你这一轮明确要在同一会话里连续推进到上限，再改用下面这句：
