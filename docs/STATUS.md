...
- 本轮继续统一日志出口：`cleanup_smoke_logging`、`trace_logging`、`cleanup_verification_docs` 的失败日志已切到 shared operational logging；`cleanup_smoke_logging` 里不可能成立的空路径兜底已删除；`feishu_long_connection` 的停止边界已收窄到 `RuntimeError`；WeCom webhook HTTP 入口把超时和一般失败分开了。
- 本轮补了 focused regression 与 shared logging primitives 守卫，守住日志标题、detail、fix hint、ANSI 去色和首个非空行摘要不漂移。
- 本轮只改日志出口、helper 命名、共享日志守卫、一个不可能成立的兜底和 WeCom webhook 超时分支，不改协议、SQLite schema、调度语义或 BT 订阅扫描行为，也没有引入新增用户可感知功能。
...
## Current health
- 默认分支质量 gate 已通过；最近一次 `make quality` 结果为 `27 passed, 0 skipped`。本轮 focused tests 覆盖 cleanup smoke、trace、cleanup docs sync 失败出口、cleanup smoke 路径兜底删除和 Feishu long connection 停止边界。
- 本轮未触发真实 downloader / refresh 协议行为变化；改动为日志出口统一、共享日志守卫补强、cleanup smoke 兜底收口、Feishu long connection 停止边界收窄，以及 WeCom webhook 超时边界分离，并由 focused tests 覆盖。
- `make quality` 已通过，`make verify-mainline` 已通过。
...
- `tests/test_wecom_adapter.py -k "webhook_handler_logs_http_timeout or webhook_handler_logs_response_write_failure or start_wecom_webhook_server_logs_bind_failure"` 通过。
