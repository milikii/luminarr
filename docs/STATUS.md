...
- 当前又从刮削后续 backlog 收掉一小格：内嵌字幕流的“中文字幕轨优先 skip / 英文可提取候选判定”也已下沉到 `subtitle_translation_support.py`，并补了“中英内嵌字幕同存时仍优先中文 skip”的 focused 护栏。当前 `subtitle_translator.py` 为 `514` 行，`_resolve_video_subtitle_files` 已收短到 `20` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `34 passed`
