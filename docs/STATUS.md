...
- 当前又从刮削后续 backlog 收掉一小格：`_translate_single_file` 与 `_extract_embedded_subtitle_file` 里的失败结果构造也已统一收口到更小 helper，避免继续在主流程里重复打印错误并组装失败结果。当前 `subtitle_translator.py` 为 `512` 行。
