...
- 当前又从刮削后续 backlog 收掉一小格：`subtitle_translator` 的翻译请求载荷构造也已下沉到 `app/services/subtitle_translation_support.py`，并补上了 `movie_title + source_lines + json_schema` 的 focused 护栏；目录型目标逐集翻译的修正继续保持有效。当前 `subtitle_translator.py` 为 `507` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `22 passed`
