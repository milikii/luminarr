...
- 本轮已把 `add_confirm_finalization_state.py` 合并回 `app/services/add_to_downloader.py`，删除只被一处消费的确认收尾薄壳。
- 本轮已把 `add_confirm_job_state.py` 合并回 `app/services/add_to_downloader.py`，删除只被一处消费的确认任务状态薄壳。
...
- 本轮主题性收口：
  ...
  - add 链单消费者状态壳已合并回 `app/services/add_to_downloader.py`：pending presence、pending write-through、confirm execution tail、pending persistence、request facade、confirm preparation、confirm availability、confirm approval、confirm context、confirm finalization、confirm job state。
...
- 本轮 focused tests：`tests/test_add_to_downloader.py -k "confirm_finalize or finalization_warning or confirm_add_by_task_ref"`，`30 passed, 82 deselected`。
- 本轮 focused tests：`tests/test_add_to_downloader.py -k "claim_pending_job or restore_pending_job or mark_completed_job or confirm_add_by_task_ref"`，`39 passed, 73 deselected`。
...
- `tests/test_add_to_downloader.py -k "confirm_finalize or finalization_warning or confirm_add_by_task_ref"` 通过（`30 passed, 82 deselected`）。
- `tests/test_add_to_downloader.py -k "claim_pending_job or restore_pending_job or mark_completed_job or confirm_add_by_task_ref"` 通过（`39 passed, 73 deselected`）。
