...
- 当前又从刮削后续 backlog 收掉一小格：`subtitle_translator` 的 chunk 翻译编排、异常校验、`ffprobe/ffmpeg` subprocess 执行编排、“可提取英文内嵌字幕结果筛选”，以及“提取后字幕文件可用性整理”已继续下沉到 `app/services/subtitle_translation_support.py`，`app/services/subtitle_translator.py` 已回落到 `488` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `18 passed`
