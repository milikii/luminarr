...
- 当前这一轮刚完成：TMDB 有候选时，搜索顺序固定为 `英文 title + 年份 -> original_title + 年份 -> 用户标题 + 年份`；不再因低置信把中文标题排前面。
...
- 搜索请求顺序现在也已固定：对中文内容和国际内容都优先走 TMDB 英文标题，再走 original_title，最后才回到中文/原始用户标题。
...
- `流浪地球2 2023` 这类真实查询里，前排完全重复的同标题结果和 `The Wandering Earth 2 / The Wandering Earth II` 这类近似重复结果，现在也会在展示前先去重，不再让同片 2160p 变体连续刷屏。
