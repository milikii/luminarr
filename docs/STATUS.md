...
- shared runtime / channel 解耦已收掉 19 条最小直连；本轮最新闭环是把 `app/bot/download_follow_up_runtime.py` 纳入 `make verify-mainline`，下载完成轮询 / 自动导入 focused 回归现在有固定质量入口，不再只靠全量 pytest 兜底。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债不再是 follow-up runtime 缺固定 gate，而是 `app/bot/private_chat_runtime.py` 仍有 `1421` 行、`app/bot/telegram_bot.py` 仍有 `1166` 行，并继续承载 BT 路由、渠道启动/停止和 Telegram 文案格式化。
- 其中更小也更直接的下一块热点是 `telegram_bot.py` 里的 webhook / personal WeChat / BT 订阅启动停止编排仍挤在同一文件；这部分比 `private_chat_runtime.py` 更容易切出下一条可验证的小闭环。
