...
- 当前下一步才开始进入本地刮削产物层：`.metadata.json` 之外的 `.nfo`、`poster`、`backdrop`。
...
- 当前最大缺口已经从“重新猜片”切到“本地刮削产物仍未开始落地”：目前还只有 `.metadata.json`，还没有最小 `.nfo` / `poster` / `backdrop` 体系。
...
- metadata / import / tmdb focused：`.venv/bin/python -m pytest -q tests/test_tmdb_client.py tests/test_metadata_scraper.py tests/test_import_to_library.py tests/test_main.py` 为 `196 passed, 4 warnings`
...
- 当前最大刮削风险已经从“导入后重新猜片”切到“本地刮削产物还没开始落地”：如果继续停在 `.metadata.json`，媒体服务器可消费的本地真相仍然不足。
- 当前机器环境真相要继续按当轮探针写；涉及内嵌字幕探测时，默认要求 `ffmpeg` 在 PATH 中可执行。
- 当前最大发布前不确定性已从“刮削输入真相边界”切到“本地刮削产物命名与写入策略”；下一步应优先做最小 `.nfo` / 图片产物，而不是再开新的 provider 支线。
