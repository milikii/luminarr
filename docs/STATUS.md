...
- 当前又从刮削后续 backlog 收掉一小格：`translate_for_import` 的 skip/success 结果构造已下沉到更小 helper；目录级 skip reason 聚合现在也已下沉到更小 helper，目录型与单文件型目标的 skip 文案、以及成功汇总文案现在都统一走更小的结果映射。当前 `subtitle_translator.py` 为 `520` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `31 passed`
