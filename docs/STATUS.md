...
- 当前又从刮削后续 backlog 收掉一小格：目录型目标在“整季每一集都已有中文字幕外挂字幕”时，现在也会返回更准确的 skip 文案，不再笼统写成“未找到可翻译字幕”；相关目录 skip 语义已收口到更小的编排段。当前 `subtitle_translator.py` 为 `537` 行，目录型 skip 结果已有 focused 护栏覆盖。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `29 passed`
