...
- 截至 2026-04-17，下载待确认轻状态查询也已补齐“持久化行缺失但进程内残留上下文”分流：`has_pending_add()` / `confirm_add_by_task_ref()` 在 `jobs` 已查不到待确认任务、但内存里还残留上下文时，会明确打印“下载待确认任务结果缺失”中文日志与 `[处理建议]`，并直接 fail-closed，不再把残留上下文误判成仍可确认下载。
