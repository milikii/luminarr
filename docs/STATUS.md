...
- 当前又从刮削后续 backlog 收掉一小格：`subtitle_translator` 的 chunk 翻译编排、异常校验与 `ffprobe` stream 解析已下沉到 `app/services/subtitle_translation_support.py`，`app/services/subtitle_translator.py` 已回落到 `521` 行。
