...
- 本轮继续统一 Telegram sidecar 日志出口：`telegram_sidecar_runtime` 的 WeCom webhook “配置不完整”日志已切到 shared operational logging。
...
## Current health
- 默认分支质量 gate 通过；本轮 focused tests 覆盖 WeCom sidecar 无效 server config 日志出口。
- 本轮未触发真实 downloader / refresh 协议行为变化；改动为 Telegram sidecar WeCom 配置日志出口统一，并由 focused tests 与 mainline gate 覆盖。
...
## Latest verification
- `tests/test_telegram_bot.py -k "test_start_wecom_webhook_server_if_configured_logs_invalid_server_config"` 通过（`1 passed`，`196 deselected`）。
