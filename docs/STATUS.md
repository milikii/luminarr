...
- 当前又从刮削后续 backlog 收掉一小格：`translate_for_import` 的“循环翻译 + 成功/跳过文案分发”也已拆成更小的编排段，并补上了 `movie_title` 成功文案、已有 `.zh` 字幕跳过文案，以及多视频目录逐集翻译的 focused 护栏。当前 `subtitle_translator.py` 为 `530` 行，但主流程入口已进一步收短到 `30` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `24 passed`
