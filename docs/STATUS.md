...
- 当前又从刮削后续 backlog 收掉一小格：英文内嵌字幕提取后的结果解析与半成品清理也已下沉到 `subtitle_translation_support.py`，并补了“提取失败时会清理半成品输出文件”的 focused 护栏。当前 `subtitle_translator.py` 为 `501` 行。
...
- 字幕 focused：`.venv/bin/python -m pytest -q tests/test_subtitle_translator.py` 为 `36 passed`
