...
- 当前 promoted 主线是 cleanup PT 最小保护窗口：只补保守阻断，不改 cleanup 删除范围、四渠道协议或 BT / raw_bt 边界。
...
- cleanup 已完成窗口证据仍成立，但 `pt_min_seed_hours` 保护正在作为新主线补最小基线。
- 字幕翻译当前仍只处理 `.srt`；`.ass` 继续留作后续能力缺口，不影响当前完成态。

## Latest verification

- 窗口活性快照：`cleanup PT 最小保护窗口` 进行中
- 当前状态快照：PT cleanup 保守阻断主线已启动
- 当前结论快照：2026-04-19 冷启动审计已确认旧“无 promoted 主线”切线收口；当前已切到新的 cleanup PT 最小保护窗口主线。
