...
- shared runtime / channel 解耦已先收掉 3 条最小直连：`raw_bt` 目录问询改为复用 `app/bot/raw_bt_destination_runtime.py`，BT TMDB 关联问询改为复用 `app/bot/bt_tmdb_association_runtime.py`，下载器角色绑定解析改为复用 `app/bot/downloader_execution_runtime.py`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债不再是测试红灯，而是 shared runtime 仍直接复用 `telegram_bot.py` 里的 execution gate / sync policy helper。
- `app/bot/private_chat_runtime.py`、`app/bot/telegram_bot.py` 仍是热点大文件；两条 BT follow-up handler 和下载器角色绑定解析都已先收口，下一段更小也更中性的边界是 `_resolve_execution_gate` / `_run_sync_with_policy`。
