...
- tests: `203 passed` (`.venv/bin/python -m pytest -q`)
...
- 为现有 BT WebSource 补最小 richer site-rule / metadata extraction baseline，继续复用共享 BT 来源适配入口，且不碰既有 approval / dispatch 路径。
