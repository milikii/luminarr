...
- 当前 search candidate helper focused 验证：`tests/test_search_media.py -k "clarification or candidate or quality_from_title"` 为 `39 passed, 64 deselected`。
...
- search candidate helper focused：`.venv/bin/python -m pytest -q tests/test_search_media.py -k "clarification or candidate or quality_from_title"` 为 `39 passed, 64 deselected`。
...
- 当前最大结构债仍在 services 层两座大山：`app/services/add_to_downloader.py` `1399` 行 / `app/services/import_to_library.py` `1392` 行；`app/services/search_media.py` 已降到 `460` 行。
- 风险消除路径：`search_media.py` 已先达标；当前最厚也最危险的是 `add_to_downloader.py` 里的 confirm state / lease / pending job 顺序控制，以及 `import_to_library.py` 的剩余执行编排壳。
