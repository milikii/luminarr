...
- `docs/TELEGRAM_BOT_SLIMMING_LOG.md`：当前“`telegram_bot.py` 渠道层瘦身 / 模块化”详细台账
- `docs/DOWNLOAD_COMPLETION_POLLING_LOG.md`：已完成的“独立后台下载完成轮询剩余少量回归与验证收口”详细台账
...
当前快照按主题归纳；当前主线具体路径、focused tests 和风险分组统一只看 `docs/TELEGRAM_BOT_SLIMMING_LOG.md`，上一条主线详细闭环继续只看 `docs/DOWNLOAD_COMPLETION_POLLING_LOG.md`，更早主线继续只看 `docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md`、`docs/FEISHU_LONG_CONNECTION_RISK_LOG.md` 和 `docs/PERSISTENCE_CLOSURE_LOG.md`，状态页不逐天或逐字段追加条目。

- 当前唯一主线已切到“`telegram_bot.py` 渠道层瘦身 / 模块化”；独立后台下载完成轮询主线已在 2026-04-18 满足 `Done when` 第 1 条，Feishu 私聊事件解析器去重、Feishu 长连接私有 API 风险收口、持久化吞错收口、cleanup 四渠道验证窗口与 shared private-chat runtime 最小抽离保持完成态。
...
- 当前新主线只收 `telegram_bot.py` 里的一个连贯切片，优先收包回包 / shared runtime wrapper，或后台生命周期 / BT pending helper，不改 shared runtime、approval、`jobs` 和 SQLite 真相；详细闭环改看 `docs/TELEGRAM_BOT_SLIMMING_LOG.md`。
...
- 当前主风险是 `app/bot/telegram_bot.py` 仍把 Telegram 收包回包、后台生命周期、BT pending helper 和 shared runtime 包装揉在同一文件里；下一步只能拆一个连贯切片，不能顺手大改。
- 当前必须继续守住已经落下来的 fail-closed 方向，不能在做 `telegram_bot.py` 瘦身时回退下载 / 导入 / 搜索 / BT 待答边界。
...
- 窗口活性快照：已满足退出条件
- 当前状态快照：进行中
- 当前结论快照：下载完成轮询主线已在 2026-04-18 通过 focused tests + 真实联调验证满足退出条件 1；当前主线已切到 `telegram_bot.py` 渠道层瘦身 / 模块化。
