...
- `docs/POST_DOWNLOAD_AUTO_IMPORT_SLIMMING_LOG.md`：刚完成的 `post_download_auto_import.py` 自动导入编排层瘦身 / 模块化台账
...
- 刚完成的 **共享状态 follow-up helper 结构降本** 已在 2026-04-20 冷启动审计中确认收口：`GetDownloadStatusService.get_status_text()` 已改成调用 `StatusFollowUpRecorder.record()` 推进共享 follow-up，focused tests 继续覆盖状态查询、状态 follow-up、后台 completion polling 与自动导入最小回归；下一条 promoted 主线切到 Telegram 后台 completion polling 直连共享 helper。
...
- 当前最小风险改成 Telegram 后台 completion polling 仍通过 `get_status_text()` 间接推进共享 follow-up；下一条 promoted 主线应优先让 `_poll_pending_download_completion_once()` 直接复用共享 helper，继续收拢这条共享职责链。
...
- 窗口活性快照：当前主线为 Telegram 后台 completion polling 直连共享 follow-up helper 收口
- 当前状态快照：Plex 这条线已按“当前主机无可达实例”收口；当前真实 refresh 测试栈仍以 Emby 为正式入口。
- 当前结论快照：近 20 条提交与当前完成态记录一致；`PERSISTENCE_CLOSURE_LOG.md` 2.1~2.4 已覆盖 10 个指定 repo 的全部对外方法，`git grep 'except Exception:\\s*\\(pass\\|return None\\)' app/services app/db app/bot` 命中仍为 `0`；共享状态 follow-up helper 结构降本已满足 `docs/NEXT_STEP.md` 退出条件 1/2/3 并收口，下一条 promoted 主线切到 Telegram 后台 completion polling 直连共享 helper。
...
- post_download_auto_import orchestration focused tests：2026-04-20，`8 passed, 34 deselected`（`.venv/bin/python -m pytest -q tests/test_get_download_status.py -k "download_monitor or auto_import_terminal"`）
