...
- 当前阶段已切到 **质量硬化**。
- 默认分支已在本轮再次复验全量回归绿灯：`.venv/bin/python -m pytest -q` 为 `1620 passed, 2 skipped`。
- shared runtime / channel 解耦已先收掉 4 条最小直连：`raw_bt` 目录问询改为复用 `app/bot/raw_bt_destination_runtime.py`，BT TMDB 关联问询改为复用 `app/bot/bt_tmdb_association_runtime.py`，下载器角色绑定解析改为复用 `app/bot/downloader_execution_runtime.py`，execution gate / sync policy helper 改为复用 `app/bot/execution_runtime.py`。
...
- 全量回归：绿灯；最近一次 `.venv/bin/python -m pytest -q` 为 `1620 passed, 2 skipped`。
...
- 全量回归：`.venv/bin/python -m pytest -q` 为 `1620 passed, 2 skipped`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债不再是测试红灯，而是 shared runtime 仍直接复用 `telegram_bot.py` 里的 frustration / BT intent / BT choice 文本解析 helper。
- `app/bot/private_chat_runtime.py`、`app/bot/telegram_bot.py` 仍是热点大文件；execution gate / sync policy helper 已先收口，下一段更小也更中性的边界是 `_is_frustration_text`、`_is_bt_direct_intent`、`_parse_bt_classification_choice`、`_parse_bt_processing_path_choice` 这一组文本解析 helper。
