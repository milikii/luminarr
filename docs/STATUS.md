...
- shared runtime / channel 解耦已先收掉 7 条最小直连：`raw_bt` 目录问询改为复用 `app/bot/raw_bt_destination_runtime.py`，BT TMDB 关联问询改为复用 `app/bot/bt_tmdb_association_runtime.py`，下载器角色绑定解析改为复用 `app/bot/downloader_execution_runtime.py`，execution gate / sync policy helper 改为复用 `app/bot/execution_runtime.py`，frustration / BT intent / BT choice 文本解析 helper 改为复用 `app/bot/query_text_runtime.py`，search reactive recovery helper 改为复用 `app/bot/search_recovery_runtime.py`，BT processing path pending state helper 改为复用 `app/bot/bt_processing_path_runtime.py`。
...
- 默认分支已恢复“全量 pytest 稳绿”，当前最大结构债不再是测试红灯，而是 shared runtime 仍直接复用 `telegram_bot.py` 里的 BT classification pending state helper。
- `app/bot/private_chat_runtime.py`、`app/bot/telegram_bot.py` 仍是热点大文件；BT processing path pending state helper 已先收口，下一段更小也更中性的边界是 `_set_bt_classification_pending`、`_is_bt_classification_pending`、`_pop_bt_classification_pending`、`_clear_bt_classification_pending` 这一组 BT classification pending state helper。
