...
- 2026-04-15 代码审查确认：`channel_identity` 空输入返回 `0` 的失败折叠、以及“查不到 `downloader_name` 仍继续查默认 Transmission”的缺口都已修掉；当前下载器路由剩余风险收口为“解析出不存在的实例名时，路由层仍可能静默回退默认下载器”。
...
- downloader name fail-closed tests：2026-04-15，`2 passed`（`.venv/bin/python -m pytest -q tests/test_main.py`）
