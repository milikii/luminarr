...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 里 BT TMDB / raw_bt follow-up 与 pending reminder 路由分支的继续瘦身**。
- 上一条已完成主线是 **`private_chat_runtime.py` 里的 BT processing-path / classification follow-up 路由分支瘦身**，当前不回退。
...
当前 **`private_chat_runtime.py` 里 BT TMDB / raw_bt follow-up 与 pending reminder 路由分支的继续瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `app/bot/private_chat_runtime.py` 当前选中的 BT TMDB / raw_bt follow-up 与 pending reminder 分支，不再继续把 pending 读取、共享 handler 调用和 reminder 回复混在同一段里；
...
1. 如果这条 BT TMDB / raw_bt follow-up 与 pending reminder 路由分支瘦身完成，就继续沿同一方向挑下一段最小边界收口点。
