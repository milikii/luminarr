...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的 runtime bootstrap helper 边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 BT pending 预检 helper 边界瘦身**：BT processing / classification follow-up 前的 query 解析、pending 读取和 service-not-ready 早退已抽到 `_resolve_bt_follow_up_precheck()`，当前不回退。
...
- `app/bot/telegram_bot.py` 已降到 `256` 行，仅保留 BT 兼容常量与 `_format_bt_classification_result()`；`app/bot/private_chat_runtime.py` 在本轮收掉 BT pending 预检 helper 后已降到 `321` 行，主函数开头的 runtime bootstrap 仍直接手写 `tg / execution_gate / traced reply` 装配。
...
- `telegram_bot.py` 纯 wrapper 已基本清空，当前更小也更直接的下一步，是继续留在 `private_chat_runtime.py` 把 runtime bootstrap 这段 `tg / execution_gate / traced reply` 装配收成最小 helper，避免 shared runtime 主函数继续背入口前置准备。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1714 passed, 2 skipped`。

## User value

- 在 BT pending 预检已经收成 helper 后，继续收 `private_chat_runtime.py` 入口最前面的 runtime bootstrap，可以让主函数更接近“只做路由编排”。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 shared runtime 主函数继续手写 `tg / execution_gate / traced reply` 这段入口装配细节。

## Only do

- 只做一次保守瘦身：围绕 `private_chat_runtime.py` 开头的 runtime bootstrap 做最小 helper 收口。
...
当前 **`private_chat_runtime.py` 的 runtime bootstrap helper 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `private_chat_runtime.py` 开头的 `tg / execution_gate / traced reply` 装配至少一段从主函数抽离，且现有文本语义不变；
2. focused tests 或既有 Telegram / shared runtime 回归能继续覆盖当前 runtime bootstrap 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `private_chat_runtime.py` 的 runtime bootstrap helper 已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
