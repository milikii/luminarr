...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的 BT pending 预检 helper 边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 dispatch wrapper 边界瘦身**：`dispatch_private_chat_text()` 已从 shared runtime 主文件移除，Telegram / Feishu / WeCom / personal WeChat 入口和相关 tests 已直接 alias 到 `handle_private_chat_query_text()`，当前不回退。
...
- `app/bot/telegram_bot.py` 已降到 `256` 行，仅保留 BT 兼容常量与 `_format_bt_classification_result()`；`app/bot/private_chat_runtime.py` 在本轮移除 dispatch wrapper 后已降到 `306` 行，但 BT follow-up 的 query 解析和 pending 预检仍直接堆在主函数里。
...
- `telegram_bot.py` 纯 wrapper 已基本清空，当前更小也更直接的下一步，是继续留在 `private_chat_runtime.py` 把 BT pending 预检这段 query 解析 + pending 读取收成最小 helper，避免 shared runtime 主函数继续背这段前置装配。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1714 passed, 2 skipped`。

## User value

- 在 shared runtime 入口别名层已经减掉后，继续收 `private_chat_runtime.py` 里 BT pending 预检这段前置装配，可以让主函数更接近“只做编排”。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 shared runtime 主函数继续手写 BT processing / classification follow-up 的 query 解析和 pending 读取细节。

## Only do

- 只做一次保守瘦身：围绕 `private_chat_runtime.py` 里 BT processing / classification follow-up 之前的 query 解析和 pending 预检做最小 helper 收口。
...
当前 **`private_chat_runtime.py` 的 BT pending 预检 helper 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `private_chat_runtime.py` 里 BT processing / classification follow-up 之前的 query 解析、pending 读取或 service-not-ready 分流至少一段从主函数抽离，且现有文本语义不变；
2. focused tests 或既有 Telegram / shared runtime 回归能继续覆盖当前 BT pending 预检最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `private_chat_runtime.py` 的 BT pending 预检 helper 已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
