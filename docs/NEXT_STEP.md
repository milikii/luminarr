...
- `app/downloader_route_lookup.py` 当前 `386` 行；task/downloader 日志上下文、payload JSON 解析收口、lookup route/client 抛错、status/remove client-only 序幕、import host `download_dir` 回填和 import source `download_dir` 重建都已共享化，downloader instance strip + lookup 与 host download_dir fallback 也已收口；当前 `_log_*` 已降到 `4` 个，剩余候选继续限制在单消费者薄壳或 route 死壳
