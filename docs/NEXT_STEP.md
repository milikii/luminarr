...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的状态查询路由边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 personal WeChat 登录路由边界瘦身**：登录路由已抽到 `app/bot/private_chat_login_runtime.py`，并把 personal WeChat 登录 focused tests 补进 `verify-mainline`，当前不回退。
...
- `app/bot/private_chat_runtime.py` 仍保留为当前 `1204` 行热点文件；personal WeChat 登录路由已抽离后，下一块更小也更贴近 shared runtime/service 边界的段落，是状态查询路由里仍直接读取 `GetDownloadStatusService` 并触发共享 follow-up。
...
- `telegram_bot.py` 的入口边界、follow-up 调度边界、生命周期编排、reply formatter 和发送 helper 都已经补过一轮；`private_chat_runtime.py` 的 personal WeChat 登录路由也已收口，当前更小也更直接的下一步，是把状态查询这段 shared runtime/service wiring 从主调度里切出去。
- 这一步继续只做最小边界瘦身，不顺手放大成新的状态平台、统一查询总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1650 passed, 2 skipped`。

## User value

- 在 personal WeChat 登录路由已经抽离并补进固定 gate 后，继续收状态查询路由，可以让 shared runtime 少直接碰 `GetDownloadStatusService` 调用细节和 follow-up 触发入口。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `private_chat_runtime.py` 的主调度函数继续同时背负 shared routing 和状态 follow-up service 调度。

## Only do

- 只做一次保守瘦身：围绕状态查询路由抽出最小 helper 或 runtime。
- 保持现有 parser / routing / approval / SQLite 真相边界不变，不新增用户可感知功能，不改渠道外部协议。
- 保持当前状态文本、follow-up 触发边界、中文日志和现有 confirm / cancel / auto-import 语义不回退。
...
- 不改状态查询协议、状态文本语义或 `GetDownloadStatusService` 的 SQLite / follow-up 真相边界。
...
当前 **`private_chat_runtime.py` 的状态查询路由边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. 状态查询路由里的 service 读取、状态执行或 reply 分发至少一段从 `private_chat_runtime.py` 抽离，且状态文本语义不变；
2. focused tests 能继续覆盖状态查询路由和当前 shared runtime 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果状态查询路由已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
