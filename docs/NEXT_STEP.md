...
- 当前质量硬化阶段的下一条最小主线切到 **`telegram_bot.py` 的 delivery / formatter 薄包装收口**。
- 刚完成的上一条主线是 **`telegram_bot.py` 的 download follow-up wrapper 边界瘦身**：`_post_download_auto_import_scheduler_loop()` / `_poll_pending_download_completion_once()` / `_download_completion_polling_loop()` 已从主文件移除，download follow-up 调度只保留在 `app/bot/download_follow_up_runtime.py`，当前不回退。
...
- `app/bot/private_chat_runtime.py` 已降到当前 `325` 行；`app/bot/telegram_bot.py` 在本轮移除 download follow-up wrapper 后已降到 `359` 行，但 `build_telegram_send_media_func()` / `build_telegram_send_text_func()` / `_format_telegram_reply()` 仍只是保留 Telegram 兼容薄包装。
...
- `private_chat_runtime.py` 的 follow-up helper 和 downloader execution wiring 已收口到 `325` 行，当前更小也更直接的下一步，是继续留在 `telegram_bot.py` 收掉剩余的 delivery / formatter 薄包装，避免 shared runtime 或测试继续经由主文件拿这些纯转发出口。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1714 passed, 2 skipped`。

## User value

- 在 `private_chat_runtime.py` 已基本收口、download follow-up wrapper 也已从 Telegram 主文件移除后，继续收 `telegram_bot.py` 里剩余的 delivery / formatter 薄包装，可以让渠道层主文件再少一段纯转发兼容层。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `telegram_bot.py` 继续同时背负 Telegram 兼容名和已独立落在 runtime / formatter 模块里的纯转发出口。

## Only do

- 只做一次保守瘦身：围绕 `telegram_bot.py` 里剩余的 delivery / formatter 薄包装做最小收口。
...
当前 **`telegram_bot.py` 的 delivery / formatter 薄包装收口** 主线视为 **已收口**，满足以下任一条即可：

1. `telegram_bot.py` 里的 `build_telegram_send_media_func()` / `build_telegram_send_text_func()` / `_format_telegram_reply()`、调用点或兼容导出至少一段从主文件抽离，且现有文本语义不变；
2. focused tests 或既有 Telegram / shared runtime 回归能继续覆盖当前 delivery / formatter 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `telegram_bot.py` 的 delivery / formatter 薄包装已收口一轮，就继续留在 `telegram_bot.py` 选下一条最小瘦身主线，或补对应 focused gate。
