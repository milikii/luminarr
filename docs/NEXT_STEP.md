...
- 当前进行中的 promoted 主线改为 **BT allowlist 分类搜索基础页 proof**。2026-04-19 当前批次已确认：上一条 **BT allowlist 分类基础页 proof** 主线满足退出条件，focused tests 已证明 `https://nyaa.si/?c=1_2` 能从命令入口直达页面抓取，并复用现有聊天缓存与 `bt批量确认` 边界；再上一条 **BT allowlist 搜索页无分类基础页 proof** 主线也保持完成态。
- 更早一条 **BT 用户页 / 编号范围页能力** 主线已在同日冷启动审计中确认满足退出条件，当前保持完成态。
- 当前继续留在 BT 而不是回 Plex：Plex 这条线已经回答了“当前不值得继续追实例”；当前更小、更保守的下一步只收 `https://nyaa.si/?f=0&c=1_2&q=frieren` 这种“已声明分类搜索页、但不带排序参数也不带分页参数”的基础页 proof，不扩站点、不放宽页面 allowlist。
...
- 当前优先交付：
  ...
  - 当前只允许补 `https://nyaa.si/?f=0&c=1_2&q=frieren` 这类已声明分类搜索基础页 proof，不扩站点、不放宽抓站边界
...
当前 BT allowlist 分类搜索基础页 proof 主线视为 **已收口**，满足以下任一条即可：

1. `bt批量 https://nyaa.si/?f=0&c=1_2&q=frieren 1-3` 已能从命令入口稳定路由到 allowlist 页面抓取，而不是退回关键词搜索；对应 focused tests 全绿；
2. 上述分类搜索基础页的只读批量预览文本已被证明可复用现有候选缓存边界；对应 focused tests 全绿；
