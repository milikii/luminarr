...
- 当前唯一主线切到 **`app/services/import_to_library.py` 数据结构重设计 · 第 12 轮 · 评估 raw_bt guard helper`**。
- 为什么现在切山：事件真相落盘已经离开主文件，`import_to_library.py` 再降 `13` 行；当前剩余更值钱、也更适合继续收口的一块，是 `_is_raw_bt_task()` 连同三条 fail-closed 中文日志。这一组继续围绕 SQLite/jobs 真相，不会碰导入协议。
...
- 当前三座大山现状：`app/services/add_to_downloader.py` `608` 行 / `import_to_library.py` `729` 行 / `search_media.py` `460` 行。
- 质量基线前置条件已满足：本轮 `tests/test_import_to_library.py` 为 `142 passed`、event focused 为 `45 passed, 97 deselected`，`make quality` 为 `24 passed`，非沙箱 `.venv/bin/python -m pytest -q` 继续 `1718 passed, 4 warnings`，真实 `/data/downloads/tr -> /data/library/movies` event recorder smoke 也已通过。

## User value

- `app/services/import_event_recorder.py` 已经把事件真相落盘拿走；`import_to_library.py` 当前 `729` 行，剩余最大结构债已经切到 raw_bt guard。
- 下一步优先评估 `_is_raw_bt_task()` 和对应中文日志，目标是把 raw_bt lookup / payload fail-closed guard 收口成 helper，不改 raw_bt 拦截协议和 SQLite 真相边界。
- 若 helper 抽离会牵动 raw_bt 判定、payload 损坏日志或查询失败边界，主线立即停住；不允许为了降行数改导入协议。

## Only do

- 只评估 `import_to_library.py` 里的 raw_bt guard helper，优先看 `_is_raw_bt_task()` 和它后面三条 fail-closed 中文日志。
- `import_to_library.py` 只继续负责用户入口、confirm 主顺序和 helper 编排；不回退已完成的 `import_prepare_state.py`、`import_event_recorder.py`、`import_confirm_preparation.py`、`import_confirm_execution_tail.py`、`import_confirm_expiry_state.py`、`import_post_processing.py`、`import_approval_state.py`、`import_job_state.py`、`import_transfer_execution.py`、`import_cancel_state.py` 和 `import_context_lookup.py` 边界。
- focused 验证优先跑 `tests/test_import_to_library.py -k \"raw_bt or context_lookup or payload_corrupted or query_failed\"`；只有在代码真的变更时才补 `make quality` 和全量 `pytest`。
...
当前 **`import_to_library.py` 数据结构重设计 · 第 12 轮 · 评估 raw_bt guard helper`** 主线视为 **已收口**，需要同时满足：

1. helper 已承接 `_is_raw_bt_task()` 至少一整块 lookup / payload fail-closed guard，`import_to_library.py` 不再直接持有这组 raw_bt 判定大块实现；
2. `app/services/import_to_library.py` 行数从 `729` 继续下降；
3. `tests/test_import_to_library.py -k \"raw_bt or context_lookup or payload_corrupted or query_failed\"` 继续绿灯；
...
1. 如果 raw_bt guard helper 抽离成功，下一条继续判断 `import_to_library.py` 里剩余的 metadata title-year / event glue 哪一段最适合再拆。
2. 如果 raw_bt guard helper 被证明会牵动 raw_bt 拦截真相或中文 fail-closed 日志，下一条改走更保守的 facade，不在导入链硬拆执行协议。
