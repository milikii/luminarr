...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的 BT batch confirm 执行 helper 边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 BT read-only helper / batch preview 路由边界瘦身**：query 解析、执行 gate 和失败日志已抽到 `app/bot/private_chat_bt_read_only_runtime.py`，并把 BT read-only focused tests 补进 `verify-mainline`，当前不回退。
...
- `app/bot/private_chat_runtime.py` 仍保留为当前 `977` 行热点文件；BT read-only helper / batch preview 已抽离后，下一块更小也更贴近 shared runtime/service 边界的段落，是 BT batch confirm 仍留在主文件里直接拿 `AddToDownloaderService`、解析 downloader 绑定并执行 gate。
...
- `telegram_bot.py` 的入口边界、follow-up 调度边界、生命周期编排、reply formatter 和发送 helper 都已经补过一轮；`private_chat_runtime.py` 的 BT read-only helper / batch preview 已收口后，当前更小也更直接的下一步，是把 BT batch confirm 这段 shared runtime/service wiring 从主文件里切出去。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1670 passed, 2 skipped`。

## User value

- 在 BT read-only helper / batch preview 已经抽离并补进固定 gate 后，继续收 BT batch confirm，可以让 shared runtime 主文件再少一段直接碰 `AddToDownloaderService`、downloader 绑定解析和执行 gate 的下载审批前置调度。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `private_chat_runtime.py` 的主调度函数继续同时背负 shared routing 和 BT 批量确认执行调度。

## Only do

- 只做一次保守瘦身：围绕 BT batch confirm 执行 helper 抽出最小 helper 或 runtime。
- 保持现有 parser / routing / approval / SQLite 真相边界不变，不新增用户可感知功能，不改渠道外部协议。
- 保持当前 BT 批量确认格式校验、downloader 绑定解析、审批文本和中文错误语义不回退。
...
- 不改 BT 批量确认协议、候选编号语义、审批文本文案或 `AddToDownloaderService` / SQLite 真相边界。
...
当前 **`private_chat_runtime.py` 的 BT batch confirm 执行 helper 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. BT batch confirm 路由里的 service 读取、格式校验、downloader 绑定解析、执行 gate 或 reply 至少一段从 `private_chat_runtime.py` 抽离，且现有文本语义不变；
2. focused tests 能继续覆盖 BT batch confirm 路由和当前 shared runtime 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 BT batch confirm 执行 helper 已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
