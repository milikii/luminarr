...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的 confirm 路由边界瘦身**。
- 刚完成的上一条主线是 **Telegram 发送 helper 边界瘦身**：媒资发送、文本发送和 Telegram 图片/文件判定已抽到 `app/bot/telegram_delivery_runtime.py`，`telegram_runtime_adapter.py` 已直接复用这层发送出口，当前不回退。
...
- `app/bot/private_chat_runtime.py` 仍保留为当前 `1421` 行热点文件；在 `telegram_bot.py` 连续收掉 formatter 和发送 helper 后，这轮回到更高风险也更有长期价值的 confirm 路由。
...
- `telegram_bot.py` 的入口边界、follow-up 调度边界、生命周期编排、reply formatter 和发送 helper 都已经补过一轮；当前更小也更直接的下一步，是回到 `private_chat_runtime.py` 收掉 confirm 路由这段共享审批编排。
- 这一步继续只做最小边界瘦身，不顺手放大成新的渠道平台、统一 webhook 框架或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1639 passed, 2 skipped`。

## User value

- 把 Telegram 生命周期编排、reply formatter 和发送 helper 连续移走后，下一步回到 `private_chat_runtime.py` 的 confirm 路由，可以优先降低审批链路维护成本，而不是继续停留在渠道文案层。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和渠道总调度耦合。
- 避免 `private_chat_runtime.py` 的 confirm 路由继续同时背负 job 关联查询、workflow 分流和 pending add fallback。

## Only do

- 只做一次保守瘦身：围绕 `confirm` 路由抽出最小 helper 或 runtime。
...
- 不调整 `confirm` 文本协议，不改 pending add / pending import 的判定真相。
...
当前 **`private_chat_runtime.py` 的 confirm 路由边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `confirm` 相关的 job 关联查询、workflow 分流或 reply helper 至少一段从 `private_chat_runtime.py` 抽离，且审批语义不变；
2. focused tests 能继续覆盖 `confirm` 路由和当前 shared runtime 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `confirm` 路由边界已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补 `confirm` focused gate。
