...
- 当前进行中的 promoted 主线改为 **BT allowlist 带分类用户显式分页 URL proof**。2026-04-20 当前批次已确认：上一条 **BT allowlist 无分类用户显式分页 URL proof** 主线满足退出条件，focused tests 已证明 `https://nyaa.si/?u=subsplease&p=2` 能从命令入口直达页面抓取，并复用现有聊天缓存与 `bt批量确认` 边界；再上一条 **BT allowlist 无分类用户排序显式分页 URL proof** 主线也保持完成态。
- 更早一条 **BT 用户页 / 编号范围页能力** 主线已在同日冷启动审计中确认满足退出条件，当前保持完成态。
- 当前继续留在 BT 而不是回 Plex：Plex 这条线已经回答了“当前不值得继续追实例”；当前更小、更保守的下一步只收 `https://nyaa.si/?f=0&c=1_2&u=subsplease&p=2` 这种“allowlist 站点用户页、带分类参数、直接显式带分页参数的基础分页页” proof，不扩站点、不放宽页面 allowlist。
- 既有 BT 页面能力已保持完成态：allowlist 页面 URL 的只读批量预览、聊天缓存、`bt批量确认` 复用 proof、category/list 页面类型，以及 `页面 URL + p=<页码>` 的最小语法糖都已补齐；当前只补这一条带分类用户显式分页 URL proof，不重做已收口页面。
...
- 当前优先交付：
  ...
  - 当前只允许补 `https://nyaa.si/?f=0&c=1_2&u=subsplease&p=2` 这类 allowlist 带分类用户显式分页 URL proof，不扩站点、不放宽抓站边界
...
当前 BT allowlist 带分类用户显式分页 URL proof 主线视为 **已收口**，满足以下任一条即可：

1. `bt批量 https://nyaa.si/?f=0&c=1_2&u=subsplease&p=2 1-3` 已能从命令入口稳定路由到 allowlist 页面抓取，而不是退回关键词搜索；对应 focused tests 全绿；
2. 上述带分类用户显式分页 URL 的只读批量预览文本已被证明可复用现有候选缓存边界；对应 focused tests 全绿；
