...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 尾段 route block 边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 execution-gated shared route block 边界瘦身**：status / watchlist / BT subscription / import / cleanup 这段连续 execution-gated shared route 已抽到 `_handle_execution_gated_shared_routes()`，当前不回退。
...
- `app/bot/telegram_bot.py` 已降到 `256` 行，仅保留 BT 兼容常量与 `_format_bt_classification_result()`；`app/bot/private_chat_runtime.py` 当前为 `382` 行，runtime bootstrap、downloader execution lambda 和 execution-gated shared route block 已收口，但 confirm / BT TMDB / raw BT destination / digit-selection / search fallback 这一段尾段 route block 仍直接堆在主函数中。
...
- `telegram_bot.py` 纯 wrapper 已基本清空，当前更小也更直接的下一步，是继续留在 `private_chat_runtime.py` 收掉尾段连续的 route block，避免 shared runtime 主函数继续背 confirm / BT follow-up / digit-selection / search fallback 这一段重复上下文透传。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1714 passed, 2 skipped`。

## User value

- 在 runtime bootstrap、downloader execution lambda 和 execution-gated shared route block 都已收口后，继续收 `private_chat_runtime.py` 尾段 route block，可以让主函数进一步靠近“只做路由编排”。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 shared runtime 主函数继续手写 confirm / BT follow-up / digit-selection / search fallback 这段重复的上下文透传细节。

## Only do

- 只做一次保守瘦身：围绕 `private_chat_runtime.py` 尾段 route block 做最小 helper 收口。
...
当前 **`private_chat_runtime.py` 尾段 route block 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `private_chat_runtime.py` 里至少一段连续的尾段 route 从主函数抽离，且现有文本语义不变；
2. focused tests 或既有 Telegram / shared runtime 回归能继续覆盖当前尾段 route helper 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `private_chat_runtime.py` 的尾段 route block 已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
