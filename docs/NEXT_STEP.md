...
- 当前唯一主线切到 **`search_media.py` media-BT 排序 / batch preview helper 收口 / 质量硬化**。
- 这一轮只服务两件事：
  - 收 `app/services/search_media.py` 里仍堆在主文件里的 media-BT 排序回退 helper、batch preview 页面支持 helper
