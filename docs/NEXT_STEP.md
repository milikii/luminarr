...
- 当前主线已从 **`javlibrary` helper 只读识别补全** 切到 **成人标题归一化回归保护**。
- 当前完成态保持：
...
- 当前下一条缺口：
  - 非 exact-id 的成人噪声标题，当前仍主要依赖站点标题、`javbus` 与已有规则
  - `javlibrary` helper 当前已故意限定在 exact-id only；下一步若继续补成人标题识别，只能先做更窄的 focused regression guard
...
- 下一步补更窄的成人标题归一化回归保护，可以在不放宽自动 dispatch 边界的前提下，降低噪声标题、大小写/分隔符变体和只读展示回退的回归风险。

## Only do

- 继续沿当前主线时，只做 **成人标题归一化回归保护** 这条更窄的小闭环：
  - 优先补 focused tests、识别 guard 和只读展示回退保护
...
当前这条 **成人标题归一化回归保护** 主线满足：

1. 成人标题的常见变体、噪声前后缀和 exact-id 输入都有 focused regression tests 保护。
...
1. 如果成人标题归一化回归保护也收口，再看是否需要补更窄的只读排序 / 展示保护。
