...
2. 下载器路由 fail-closed：`channel_identity` 空输入返回 `0`、缺少 `downloader_name`、以及下载投递 path 里的非法显式实例名回退默认下载器都已修掉，lookup 未命中现在也会打印中文日志；当前剩余的是“lookup 路径里命中未知实例名时仍缺显式中文日志”。
