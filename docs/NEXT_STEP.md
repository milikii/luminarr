...
- `app/downloader_route_lookup.py` 当前 `404` 行；task/downloader 日志上下文、payload JSON 解析收口、lookup route/client 抛错、status/remove client-only 序幕、import host `download_dir` 回填和 import source `download_dir` 重建都已共享化，downloader instance strip + lookup 与 host download_dir fallback 也已收口；当前 `_log_*` 只剩 dispatch 专用 helper 1 个，剩余候选继续限制在单消费者薄壳或 route 死壳
- `app/config.py` 当前 `461` 行；base URL 现在统一走 shared normalization helper，继续保留既有的 semicolon / pipe 解析收口，不回头扩张 settings 语义。
