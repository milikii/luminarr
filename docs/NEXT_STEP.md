...
- 当前质量硬化阶段的下一条最小主线切到 **`telegram_bot.py` 的 download follow-up wrapper 边界瘦身**。
- 刚完成的上一条主线是 **`telegram_bot.py` 的 BT entry helper 边界瘦身**：`_enter_pure_bt_flow()` / `_enter_media_import_bt_flow()` 已抽到 `app/bot/telegram_bt_entry_runtime.py`，`verify-mainline` 已补进 Telegram BT entry focused tests，当前不回退。
...
- `app/bot/private_chat_runtime.py` 已降到当前 `325` 行；`app/bot/telegram_bot.py` 在 BT entry helper 收口后已降到 `399` 行，但 download follow-up 的 3 个 scheduler wrapper 仍只是保留 Telegram 兼容名再转发到 `app/bot/download_follow_up_runtime.py`。
...
- `private_chat_runtime.py` 的 follow-up helper 和 downloader execution wiring 已收口到 `325` 行，当前更小也更直接的下一步，是继续留在 `telegram_bot.py` 把 download follow-up wrapper 这段渠道侧兼容层从主文件里切出去。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1711 passed, 2 skipped`。

## User value

- 在 `private_chat_runtime.py` 已基本收口、Telegram BT 相关薄包装也连续收掉后，继续收 `telegram_bot.py` 里的 download follow-up wrapper，可以让渠道层主文件再少保留一段纯转发兼容层。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `telegram_bot.py` 继续同时背负 Telegram 兼容名和已拆出去的 follow-up runtime 转发层。

## Only do

- 只做一次保守瘦身：围绕 `telegram_bot.py` 的 download follow-up wrapper 抽出最小 helper 或 runtime。
...
当前 **`telegram_bot.py` 的 download follow-up wrapper 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `telegram_bot.py` 里的 `_post_download_auto_import_scheduler_loop()` / `_poll_pending_download_completion_once()` / `_download_completion_polling_loop()`、调用点或兼容导出至少一段从主文件抽离，且现有文本语义不变；
2. focused tests 或既有 Telegram / shared runtime 回归能继续覆盖当前 download follow-up wrapper 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `telegram_bot.py` 的 download follow-up wrapper 已收口一轮，就继续留在 `telegram_bot.py` 选下一条最小瘦身主线，或补对应 focused gate。
