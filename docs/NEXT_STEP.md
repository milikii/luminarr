...
- 当前质量硬化阶段的下一条最小主线切到 **`telegram_bot.py` 的 Telegram entry wrapper 边界瘦身**。
- 刚完成的上一条主线是 **`telegram_bot.py` 的 delivery / formatter 薄包装收口**：`build_telegram_send_media_func()` / `build_telegram_send_text_func()` / `_format_telegram_reply()` 已从主文件移除，`telegram_runtime_adapter.py` 与 tests 已直接复用 `telegram_delivery_runtime.py` / `telegram_reply_formatter.py`，当前不回退。
...
- `app/bot/private_chat_runtime.py` 已降到当前 `325` 行；`app/bot/telegram_bot.py` 在本轮移除 delivery / formatter 薄包装后已降到 `338` 行，但 `handle_message()` / `handle_callback_query()` / `build_application()` 仍只是保留 Telegram entry 兼容薄包装再转发到 `app/bot/telegram_runtime_adapter.py`。
...
- `private_chat_runtime.py` 的 follow-up helper 和 downloader execution wiring 已收口到 `325` 行，当前更小也更直接的下一步，是继续留在 `telegram_bot.py` 收掉剩余的 Telegram entry 兼容包装，避免渠道入口仍经由主文件转发到 `telegram_runtime_adapter.py`。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1714 passed, 2 skipped`。

## User value

- 在 `private_chat_runtime.py` 已基本收口、delivery / formatter 薄包装也已从 Telegram 主文件移除后，继续收 `telegram_bot.py` 里剩余的 entry wrapper，可以让渠道层主文件进一步只保留常量与 BT pending / entry helper。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `telegram_bot.py` 继续同时背负 Telegram 兼容名和已独立落在 `telegram_runtime_adapter.py` 的入口转发出口。

## Only do

- 只做一次保守瘦身：围绕 `telegram_bot.py` 里剩余的 Telegram entry wrapper 做最小收口。
...
当前 **`telegram_bot.py` 的 Telegram entry wrapper 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `telegram_bot.py` 里的 `handle_message()` / `handle_callback_query()` / `build_application()`、调用点或兼容导出至少一段从主文件抽离，且现有文本语义不变；
2. focused tests 或既有 Telegram / shared runtime 回归能继续覆盖当前 entry wrapper 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `telegram_bot.py` 的 Telegram entry wrapper 已收口一轮，就继续留在 `telegram_bot.py` 选下一条最小瘦身主线，或补对应 focused gate。
