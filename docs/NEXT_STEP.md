...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的 BT direct-intent pending reset helper 边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 frustration cancel / reset helper 边界瘦身**：取消路由里的 service/repo 分流、执行 gate 和 BT pending 清理已抽到 `app/bot/private_chat_frustration_runtime.py`，并把 frustration focused tests 补进 `verify-mainline`，当前不回退。
...
- `app/bot/private_chat_runtime.py` 仍保留为当前 `715` 行热点文件；frustration cancel / reset helper 已抽离后，下一块更小也更贴近 shared runtime/service 边界的段落，是 BT direct-intent pending reset helper 仍留在主文件里直接清理多段 BT pending 状态并写回新的 processing-path 待处理。
...
- `telegram_bot.py` 的入口边界、follow-up 调度边界、生命周期编排、reply formatter 和发送 helper 都已经补过一轮；`private_chat_runtime.py` 的 frustration helper 已收口后，当前更小也更直接的下一步，是把 BT direct-intent pending reset 这段 shared runtime wiring 从主文件里切出去。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1670 passed, 2 skipped`。

## User value

- 在 frustration helper 已经抽离并补进固定 gate 后，继续收 BT direct-intent pending reset，可以让 shared runtime 主文件再少一段直接碰 BT pending state 的清理与重建。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `private_chat_runtime.py` 的主调度函数继续同时背负 shared routing 和 BT direct-intent 的 pending reset 调度。

## Only do

- 只做一次保守瘦身：围绕 BT direct-intent pending reset helper 抽出最小 helper 或 runtime。
- 保持现有 parser / routing / approval / SQLite 真相边界不变，不新增用户可感知功能，不改渠道外部协议。
- 保持当前 magnet / BT 直接入口语义、processing-path 提示文案、BT pending 清理顺序和中文错误语义不回退。
...
- 不改 magnet 入口协议、处理链提示、BT pending / approval / SQLite 真相边界。
...
当前 **`private_chat_runtime.py` 的 BT direct-intent pending reset helper 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. BT direct-intent 路由里的 pending 清理、processing-path 待处理写入或 reply 至少一段从 `private_chat_runtime.py` 抽离，且现有文本语义不变；
2. focused tests 能继续覆盖 BT direct-intent 路由和当前 shared runtime 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 BT direct-intent pending reset helper 已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
