...
当前 **热点大文件里 BT 批量确认路由分支的继续瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `app/bot/private_chat_runtime.py` 当前选中的 BT 批量确认路由分支，不再继续把编号校验、service 获取、下载器绑定解析和 reply 编排混在同一段里；
...
1. 如果这条 BT 批量确认路由分支瘦身完成，就继续沿同一方向挑下一段最小边界收口点。
