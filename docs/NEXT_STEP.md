...
3. 持久化吞错收口：`search_media` 里的搜索候选持久化失败、澄清态 `upsert_pending()` / `clear_pending()` / `get_pending_query()` 失败、候选读取 `get_candidate()` 失败，以及 `add_to_downloader.has_pending_add()` / `cancel_pending_add()` / `_rebuild_confirm_context()` 里的 `get_downloader_job_for_chat_ref()` / `get_latest_pending_downloader_job()` / `get_downloader_approval()` 查询失败，都已经补上显式中文日志与 `[处理建议]`；当前剩余的是其他持久化路径里的 `except Exception: pass/return None`，仍需继续改成“区分真缺数据和 SQLite / 配置异常”的显式日志。
