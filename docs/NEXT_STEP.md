...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的 cleanup 路由边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 BT subscription 路由边界瘦身**：BT subscription 路由已抽到 `app/bot/private_chat_bt_subscription_runtime.py`，并把 subscription focused tests 补进 `verify-mainline`，当前不回退。
...
- `app/bot/private_chat_runtime.py` 仍保留为当前 `1161` 行热点文件；BT subscription 路由已抽离后，下一块更小也更贴近 shared runtime/service 边界的段落，是 cleanup inspect / cleanup 执行分支仍留在主调度里直接拼 `cleanup_runner` 和 side-effect action。
...
- `telegram_bot.py` 的入口边界、follow-up 调度边界、生命周期编排、reply formatter 和发送 helper 都已经补过一轮；`private_chat_runtime.py` 的 BT subscription 路由也已收口，当前更小也更直接的下一步，是把 cleanup inspect / cleanup 这段 shared runtime/service wiring 从主调度里切出去。
- 这一步继续只做最小边界瘦身，不顺手放大成新的 cleanup 平台、统一 side-effect 总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1666 passed, 2 skipped`。

## User value

- 在 BT subscription 路由已经抽离并补进固定 gate 后，继续收 cleanup 路由，可以让 shared runtime 少直接碰 cleanup action 选择、service 调用和 side-effect 分支。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `private_chat_runtime.py` 的主调度函数继续同时背负 shared routing 和 cleanup side-effect 调度。

## Only do

- 只做一次保守瘦身：围绕 cleanup inspect / cleanup 路由抽出最小 helper 或 runtime。
- 保持现有 parser / routing / approval / SQLite 真相边界不变，不新增用户可感知功能，不改渠道外部协议。
- 保持当前 cleanup 文本、中文日志和现有 side-effect / inspect 语义不回退。
...
- 不改 cleanup 协议、文本文案或 `CleanupDownloadedSourceService` 的 side-effect / SQLite 真相边界。
...
当前 **`private_chat_runtime.py` 的 cleanup 路由边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. cleanup inspect / cleanup 路由里的 service 读取、action 分流、cleanup runner 组装或 reply 分发至少一段从 `private_chat_runtime.py` 抽离，且 cleanup 文本语义不变；
2. focused tests 能继续覆盖 cleanup 路由和当前 shared runtime 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 cleanup 路由已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
