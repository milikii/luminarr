...
- 为什么现在切山：`import_to_library.py` 的 context / approval / jobs / file-transfer / cancel 五段都已经收口，再继续微拆很容易落进 AGENTS 的收益递减区；`add_to_downloader.py` 仍有 `1117` 行，依旧是 services 层当前最大的单文件之一。
...
- 当前三座大山现状：`app/services/add_to_downloader.py` `1117` 行 / `import_to_library.py` `1392` 行 / `search_media.py` `460` 行。
...
- `add_to_downloader.py` 里的 jobs 状态机、approval / lease 查询和 confirm context / expiry 收口都已经离开主文件，但 pending approval 的写入 / approve / restore / cancel / executed-version 回写还混在同一壳里。
- 下一步优先评估 pending approval persistence helper，目的是把“确认编排顺序”和“approval_record + 进程内 lease 身份维护”拆开，继续收紧下载确认前后的高风险真相写入链。
- 若 helper 抽离会牵动 approval_record 主键迁移、lease/version 真相或下载副作用边界，主线立即停住；不允许为了降行数改下载协议。

## Only do

- 只评估 `add_to_downloader.py` 里的 pending approval persistence helper，优先看 `_record_pending_approval()`、`_record_downloader_approval()`、`_restore_pending_approval()`、`_cancel_pending_approval()`、`_record_executed_lease_version()` 与它们依赖的最小 identity 维护。
...
当前 **`add_to_downloader.py` 数据结构重设计 · 第 12 轮 · 评估 pending approval persistence helper`** 主线视为 **已收口**，需要同时满足：

1. helper 已承接 pending approval 写入 / restore / cancel / executed-version 回写至少一整块边界，`add_to_downloader.py` 不再直接持有这组大块实现；
2. `app/services/add_to_downloader.py` 行数从 `1117` 继续下降；
...
1. 如果 pending approval persistence helper 抽离成功，下一条继续评估这座山里剩余 finalization warning / approval identity move 是否还有继续拆的价值。
2. 如果 pending approval persistence helper 被证明会牵动 approval / lease 真相，下一条改走更保守的 facade，不直接外提整组状态写入。
