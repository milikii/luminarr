...
- 为什么现在切山：request 入口已经离开主文件，接下来最厚的一整块只剩 confirm 编排壳；继续顺着这块顺序控制做 facade，仍然能明显降协调密度。
...
- 当前三座大山现状：`app/services/add_to_downloader.py` `763` 行 / `import_to_library.py` `1392` 行 / `search_media.py` `460` 行。
...
- `add_to_downloader.py` 里的 add request facade、jobs 状态机、approval / lease 查询、confirm context / expiry、pending approval persistence、approval identity move、confirm finalization、pending runtime state、pure trace wrapper 和 pending reply/persistence helper 都已经离开主文件，但 confirm 编排壳还集中挂在主文件中段。
- 下一步优先评估 confirm 编排壳，目的是继续把“顺序控制”和“状态判定 / 副作用回退组合”拆开，先判断 `confirm_add_by_task_ref()` 里 claim / stale-check / approve / dispatch-failure restore 这组序列是否还能共用一层 facade。
- 若 helper 抽离会牵动 shared runtime、trace 协议或下载副作用边界，主线立即停住；不允许为了降行数改下载协议。

## Only do

- 只评估 `add_to_downloader.py` 里的 confirm 编排壳，优先看 `confirm_add_by_task_ref()` 里 claim pending job、stale-check、approval 更新、dispatch 失败回退和 finalization 之间的重复顺序控制。
- `add_to_downloader.py` 只继续负责用户入口、confirm 编排和 helper 顺序控制；不回退已完成的 `add_request_facade.py`、`add_pending_context.py`、`add_pending_persistence.py`、`add_trace_logger.py`、`add_execution_follow_up.py`、`add_cancel_state.py`、`add_confirm_job_state.py`、`add_confirm_approval_state.py` 和 `add_confirm_context_state.py` 边界。
...
当前 **`add_to_downloader.py` 数据结构重设计 · 第 19 轮 · 评估 confirm 编排壳`** 主线视为 **已收口**，需要同时满足：

1. helper 已承接 confirm 编排壳至少一整块边界，`add_to_downloader.py` 不再直接持有这组顺序控制的大块实现；
2. `app/services/add_to_downloader.py` 行数从 `763` 继续下降；
...
1. 如果 confirm 编排壳抽离成功，下一条继续评估这座山是否已经逼近收益递减，还是还能再收一块最小顺序控制辅助。
2. 如果 confirm 编排壳被证明会牵动 approval / jobs / dispatch 回退真相，下一条改走更保守的 facade，不直接外提整组 confirm 顺序控制。
