...
3. 持久化吞错收口：`search_media` 里的搜索候选持久化失败已经补上显式中文 `[搜索候选持久化失败]` 和 `[处理建议]`；当前剩余的是候选读取、澄清态 upsert/clear/load，以及下载器路由等路径里的 `except Exception: pass/return None`，仍需继续改成“区分真缺数据和 SQLite / 配置异常”的显式日志。
