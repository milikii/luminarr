...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 里 BT processing-path / classification follow-up 路由分支的继续瘦身**。
- 上一条已完成主线是 **`private_chat_runtime.py` 里的 frustration / reset 路由分支瘦身**，当前不回退。
...
当前 **`private_chat_runtime.py` 里 BT processing-path / classification follow-up 路由分支的继续瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `app/bot/private_chat_runtime.py` 当前选中的 processing-path / classification follow-up 分支，不再继续把 pending pop、跨 stage 清理和流程进入逻辑混在同一段里；
...
1. 如果这条 processing-path / classification follow-up 路由分支瘦身完成，就继续沿同一方向挑下一段最小边界收口点。
