...
- 当前质量硬化阶段的下一条最小主线切到 **`telegram_bot.py` 的 bound downloader execution helper 边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 bound downloader execution resolver helper 边界瘦身**：downloader role binding / 实例解析已抽到 `app/bot/private_chat_downloader_execution_runtime.py`，confirm / digit-selection 薄包装已从主文件移除，并把 resolver focused tests 补进 `verify-mainline`，当前不回退。
...
- `app/bot/private_chat_runtime.py` 已降到当前 `325` 行；bound downloader execution resolver 和 confirm / digit-selection 薄包装收口后，当前更大的热点已经回到 `app/bot/telegram_bot.py`，其中 bound downloader execution helper 仍直接握着 Telegram context、角色绑定和实例解析。
...
- `private_chat_runtime.py` 的 follow-up helper 和 downloader execution wiring 已收口到 `325` 行，当前更小也更直接的下一步，是回到 `telegram_bot.py` 把 bound downloader execution helper 这段渠道侧 wiring 从主文件里切出去。
- 这一步继续只做最小边界瘦身，不顺手放大成新的搜索平台、统一回复总线或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1711 passed, 2 skipped`。

## User value

- 在 `private_chat_runtime.py` 已基本收口后，回到 `telegram_bot.py` 继续收 bound downloader execution helper，可以让渠道层主文件再少一段直接碰 Telegram context、下载器角色绑定和实例解析。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和 shared runtime/service 耦合。
- 避免 `telegram_bot.py` 的消息入口继续同时背负渠道协议处理和 downloader execution wiring。

## Only do

- 只做一次保守瘦身：围绕 `telegram_bot.py` 的 bound downloader execution helper 抽出最小 helper 或 runtime。
...
当前 **`telegram_bot.py` 的 bound downloader execution helper 边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `telegram_bot.py` 里的 downloader execution helper、调用点或错误透传至少一段从主文件抽离，且现有文本语义不变；
2. focused tests 或既有 Telegram / shared runtime 回归能继续覆盖当前 downloader execution wiring 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 `telegram_bot.py` 的 bound downloader execution helper 已收口一轮，就继续留在 `telegram_bot.py` 选下一条最小瘦身主线，或补对应 focused gate。
