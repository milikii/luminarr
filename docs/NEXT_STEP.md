...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 尾部 confirm / 数字选项 / 搜索 fallback 路由分支的继续瘦身**。
- 上一条已完成主线是 **`private_chat_runtime.py` 里的 BT TMDB / raw_bt follow-up 与 pending reminder 路由分支瘦身**，当前不回退。
...
- 把 `private_chat_runtime.py` 尾部仍然混在一起的 confirm 路由、数字选项投递和搜索 fallback 再拆薄一点，减少后续调整尾部路由顺序或 PT/BT 分流时误伤其他分支的概率。
- 在默认分支已经稳绿的前提下，优先继续降低热点文件耦合，避免未来回归再次堆到 `telegram_bot.py` 和 `private_chat_runtime.py`。

## Only do

- 只做 `private_chat_runtime.py` 尾部 confirm / 数字选项 / 搜索 fallback 路由分支的最小瘦身。
...
当前 **`private_chat_runtime.py` 尾部 confirm / 数字选项 / 搜索 fallback 路由分支的继续瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. `app/bot/private_chat_runtime.py` 当前选中的尾部路由分支，不再继续把 confirm 分流、数字选项投递和搜索 fallback 混在同一段里；
...
1. 如果这条尾部 confirm / 数字选项 / 搜索 fallback 路由分支瘦身完成，就继续沿同一方向挑下一段最小边界收口点。
