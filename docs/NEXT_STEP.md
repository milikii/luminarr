...
- 这一轮只允许先抽 `_record_pending_job()`、`_claim_pending_job()`、`_restore_pending_job()`、`_mark_completed_job()` 这一组；approval helper、copy-fallback payload、文件导入执行和 `cancel_pending_import()` 先不动。
...
- 当前最大结构债转移到 services 层三座大山：`import_to_library.py` `1827` 行 / `add_to_downloader.py` `1669` 行 / `search_media.py` `1018` 行，合计 `4514` 行，占全仓 `25663` 行的 `17.6%`。当前仍只动最大的一座，另两座留待后续独立主线。
...
- approval 现在已经从主文件拿走，下一块最危险的就是 `jobs` 状态迁移：pending upsert、lease claim、release 和 mark completed 仍直接挤在 confirm 编排里。
- 先把 jobs helper 抽出来，可以把“审批真相”和“任务状态机”拆成两个独立读写面，后面再收 `copy-fallback` / 文件导入执行时更不容易互相带坏。
- 若 helper 抽离后被迫改变 `confirm` / `import` 的回复语义或 `jobs` 状态迁移顺序，主线立即停住；不允许为了降行数重排状态机。

## Only do

- 只抽 jobs 状态迁移 helper，例如 `app/services/import_job_state.py`，目标函数优先包括 `_record_pending_job()`、`_claim_pending_job()`、`_restore_pending_job()`、`_mark_completed_job()`。
- `app/services/import_to_library.py` 只允许继续负责 confirm 编排、reply 决策和 helper 顺序控制；jobs helper 自己承接 `jobs` pending/lease/completed 状态迁移和 fail-closed 中文日志。
...
- 不在这一轮回退或改写 approval helper，不改 stale / expiry / pending lease 查询边界。
...
当前 **`import_to_library.py` 数据结构重设计 · 第 4 轮 · 抽离 jobs state transition helper`** 主线视为 **已收口**，需要同时满足：

1. jobs helper 已承接 pending/claim/release/complete 这一组状态迁移和中文 fail-closed 日志，`import_to_library.py` 不再直接持有这组大块实现；
2. `app/services/import_to_library.py` 行数从 `1827` 再下降，目标优先看 `≤ 1650`；
...
1. 如果 jobs helper 抽离成功，下一条主线切到 **copy-fallback / 文件导入执行 helper**，把 confirm 主链里剩余的文件系统分支再压掉一段。
2. 如果 jobs helper 抽离被证明与 confirm 编排耦合过深，下一条改走更保守的 `ImportJobStateTransition` facade，不直接把写路径全部外提。
