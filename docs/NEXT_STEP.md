...
- 当前唯一主线：**shared private-chat 交付体验收口**（2026-04-19 已在 `series / anime` 主线通过 `_extract_title_year_for_scrape()` + focused suite `245 passed` 满足退出条件 1 后切到本线）
...
- 当前主线的设计蓝图统一写在 `docs/SHARED_DELIVERY_UX_PLAN.md`
- 当前主线的详细闭环、focused tests 和风险分组统一写在 `docs/SHARED_DELIVERY_UX_LOG.md`
- 上一条已完成主线“`series / anime` 独立名称解析最小实现”详细闭环继续写在 `docs/SERIES_ANIME_NAMING_LOG.md`
...
- 继续收口 shared private-chat 交付体验的一个最小 Phase；每轮只做一个最小闭环，不顺手清理不相关模块
...
- 涉及真实 downloader / import / refresh 行为的任务，继续使用本地 Transmission / Emby 联调栈验证；当前主线优先做内容模型和 renderer，不扩成新的下载、导入或刷新副作用闭环
- 文档继续分层：
  - `docs/STATUS.md` 只保留当前快照
  - `docs/SHARED_DELIVERY_UX_PLAN.md` 承接当前主线蓝图
  - `docs/SHARED_DELIVERY_UX_LOG.md` 承接当前主线详细闭环
  - `docs/SERIES_ANIME_NAMING_LOG.md` 保留上一条主线已完成台账
...
- 新闭环按主题合并进 `docs/SHARED_DELIVERY_UX_LOG.md` 2.1~2.3 已有分组，**不再开新的 `### 2026-xx-xx 分流缺口` 小节；`STATUS.md` 只补一句当前结论或风险，不逐天追加 `截至 ...` 条目**
...
- 不把 shared private-chat 交付体验主线借机扩成 Web UI、通用前端平台、通用模板引擎或新的多渠道框架
- 不放宽现有 pending state gate、approval、`jobs` / `job_event` / lease/version / SQLite 真相边界
- 不在这一步引入自动 `confirm`、自动下载、新的下载/导入 workflow、Jellyfin / Plex 支持或其他新集成
...
1. `search_results` / `approval` / `status` / `error` 四类核心消息在 Telegram / personal WeChat / Feishu / WeCom 四渠道各有对应 renderer，且 `.venv/bin/python -m pytest -q tests/test_delivery_renderers.py tests/test_search_media.py tests/test_add_to_downloader.py tests/test_get_download_status.py` 全绿；
2. 或者 `docs/SHARED_DELIVERY_UX_PLAN.md` Phase 1-6 已完成 4 个，剩余的都属于排版精调，此时停下来请用户确认；
3. 或者本轮代码改动 **< 20 行**、只是微调 emoji / 空格（收益递减），此时走 `AGENTS.md §11` 停机规则。

满足其中任一条时，直接回到本文件 `After this step` 第 1 项（最小人类可用入口继续补齐）。

附加约束（不算退出条件，只是不得违反）：

- 四渠道现有 cleanup / search / approval / import / status / watchlist / btsub 协议不回退
- `verification docs gate` 与 cleanup 完成态证据持续通过
- `docs/STATUS.md` / `docs/NEXT_STEP.md` / `README.md` / `docs/SHARED_DELIVERY_UX_PLAN.md` / `docs/SHARED_DELIVERY_UX_LOG.md` / `docs/SERIES_ANIME_NAMING_LOG.md` / `docs/APP_MAIN_SLIMMING_LOG.md` / `docs/PRIVATE_CHAT_RUNTIME_SLIMMING_LOG.md` / `docs/CLEANUP_SLIMMING_LOG.md` / `docs/MANAGE_BT_SUBSCRIPTION_SLIMMING_LOG.md` / `docs/SEARCH_MEDIA_SLIMMING_LOG.md` / `docs/ADD_TO_DOWNLOADER_SLIMMING_LOG.md` / `docs/IMPORT_TO_LIBRARY_SLIMMING_LOG.md` / `docs/TELEGRAM_BOT_SLIMMING_LOG.md` / `docs/DOWNLOAD_COMPLETION_POLLING_LOG.md` / `docs/FEISHU_EVENT_PARSER_DEDUPE_LOG.md` / `docs/FEISHU_LONG_CONNECTION_RISK_LOG.md` / `docs/PERSISTENCE_CLOSURE_LOG.md` / `docs/CLEANUP_VERIFICATION_WINDOW.md` 继续保持分层一致，不重新写回长台账

## After this step

1. 最小人类可用入口继续补齐（quick start / 配置模板 / 首个渠道 10 分钟跑通）——详细蓝图见 `docs/QUICK_START_PLAN.md`
2. BT 共享确定性评分器——详细蓝图见 `docs/BT_SCORING_PLAN.md`
3. Jellyfin / Plex 支持（后续）
4. plugin 体系继续后置
