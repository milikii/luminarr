...
4. `cleanup_smoke_logging` 去模块级全局状态：目录创建失败返回伪成功路径的问题已修掉，`log_cleanup_private_chat_smoke()` 已支持显式 `log_path` 传参，测试侧也已有 reset helper 复位默认路径；运行时默认落盘路径也已改成调用时本地解析，且追加链已经不再读取模块级 `_cleanup_private_chat_smoke_log_path`；当前剩余的是 configure/reset helper 自身仍保留这份模块级状态。
