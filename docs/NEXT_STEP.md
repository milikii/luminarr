...
4. `cleanup_smoke_logging` 去模块级全局状态：目录创建失败返回伪成功路径的问题已修掉，`log_cleanup_private_chat_smoke()` 已支持显式 `log_path` 传参，测试侧也已有 reset helper 复位默认路径；当前剩余的是运行时默认路径仍回退到模块级 `_cleanup_private_chat_smoke_log_path` 全局状态。
