...
- 当前唯一主线切到 **`app/services/add_to_downloader.py` 数据结构重设计 · 第 18 轮 · 评估 add request 入口壳`**。
- 为什么现在切山：pending reply / persistence 壳已经离开 `add_to_downloader.py`，主文件顶部仍有一组很集中的 add request 入口方法；继续顺着这组入口 facade 收口，仍然是当前最小切口。
...
- 当前三座大山现状：`app/services/add_to_downloader.py` `787` 行 / `import_to_library.py` `1392` 行 / `search_media.py` `460` 行。
...
- `add_to_downloader.py` 里的 jobs 状态机、approval / lease 查询、confirm context / expiry、pending approval persistence、approval identity move、confirm finalization、pending runtime state、pure trace wrapper 和 pending reply/persistence helper 都已经离开主文件，但 add request 入口壳还集中挂在主文件顶部。
- 下一步优先评估 add request 入口壳，目的是继续把“用户入口”和“候选构建 + persist 调用重复逻辑”拆开，先判断 `add_by_selection()`、`add_by_batch_selection()`、`add_candidate_source()` 和 `add_bt_source()` 是否还能共用一层 facade。
- 若 helper 抽离会牵动 shared runtime、trace 协议或下载副作用边界，主线立即停住；不允许为了降行数改下载协议。

## Only do

- 只评估 `add_to_downloader.py` 里的 add request 入口壳，优先看 `add_by_selection()`、`add_by_batch_selection()`、`add_candidate_source()`、`add_bt_source()` 与 build-result -> persist 的重复路径。
- `add_to_downloader.py` 只继续负责用户入口、confirm 编排和 helper 顺序控制；不回退已完成的 `add_pending_context.py`、`add_pending_persistence.py`、`add_trace_logger.py`、`add_execution_follow_up.py`、`add_cancel_state.py`、`add_confirm_job_state.py`、`add_confirm_approval_state.py` 和 `add_confirm_context_state.py` 边界。
...
当前 **`add_to_downloader.py` 数据结构重设计 · 第 18 轮 · 评估 add request 入口壳`** 主线视为 **已收口**，需要同时满足：

1. helper 已承接 add request 入口壳至少一整块边界，`add_to_downloader.py` 不再直接持有这组入口重复路径的大块实现；
2. `app/services/add_to_downloader.py` 行数从 `787` 继续下降；
...
1. 如果 add request 入口壳抽离成功，下一条继续评估剩余 confirm 编排壳是否已经逼近收益递减。
2. 如果 add request 入口壳被证明会牵动 BT 边界、选择序号协议或 shared delivery 文本协议，下一条改走更保守的 facade，不直接外提整组入口辅助。
