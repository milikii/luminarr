...
- 当前质量硬化阶段的下一条最小主线切到 **`telegram_bot.py` 里 webhook / personal WeChat / BT 订阅启动停止编排边界瘦身**。
- 当前这条主线只观察 **Telegram 渠道生命周期编排**，优先把 Feishu/WeCom webhook、personal WeChat 和 BT 订阅这些 sidecar 启停 helper 再收一层，减少 `telegram_bot.py` 继续充当总调度桶。
...
- `telegram_bot.py` 的入口边界、follow-up 调度边界和固定质量 gate 都已经补过一轮；当前更小也更直接的下一步，是继续削掉同一文件里的生命周期编排块。
- 这一步继续只做最小边界瘦身，不顺手放大成新的渠道平台、统一 webhook 框架或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1632 passed, 2 skipped`。

## User value

- 把 webhook / personal WeChat / BT 订阅这些 sidecar 启停从 Telegram 主文件里再往外收一层，后续排查启动顺序和停机清理时不必同时读 BT 路由和 Telegram 文案格式化。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和渠道总调度耦合。
- 避免 `telegram_bot.py` 在入口、follow-up、生命周期三条链都刚拆完后又继续回涨。

## Only do

- 只做一次保守瘦身：围绕 Telegram 渠道生命周期 helper 抽出最小 runtime 或薄封装。
...
当前 **`telegram_bot.py` 里 webhook / personal WeChat / BT 订阅启动停止编排边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. Telegram 渠道生命周期里至少一段 sidecar 启停 helper 从 `telegram_bot.py` 抽离，且启动/停止顺序语义不变；
