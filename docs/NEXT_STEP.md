...
- 当前质量硬化阶段的下一条最小主线切到 **`private_chat_runtime.py` 的 personal WeChat 登录路由边界瘦身**。
- 刚完成的上一条主线是 **`private_chat_runtime.py` 的 trace 包装边界瘦身**：trace 路径解析、入站日志和 reply trace 包装已抽到 `app/bot/private_chat_trace_runtime.py`，`verify-mainline` 已补进 trace focused tests，当前不回退。
...
- `app/bot/private_chat_runtime.py` 仍保留为当前 `1215` 行热点文件；trace 包装已抽离后，下一块更小也更贴近渠道耦合的段落，是 personal WeChat 登录路由里仍直接读取 Telegram 发送 helper 和登录 service。
...
- `telegram_bot.py` 的入口边界、follow-up 调度边界、生命周期编排、reply formatter 和发送 helper 都已经补过一轮；`private_chat_runtime.py` 的 trace 包装也已收口，当前更小也更直接的下一步，是把 personal WeChat 登录这段渠道私有 wiring 从 shared runtime 主调度里切出去。
- 这一步继续只做最小边界瘦身，不顺手放大成新的渠道平台、统一登录平台或大文件总重写。
- 质量基线前置条件已满足：默认分支本轮复验 `.venv/bin/python -m pytest -q` 为 `1647 passed, 2 skipped`。

## User value

- 在 trace 包装已经抽离并补进固定 gate 后，继续收 personal WeChat 登录路由，可以让 shared runtime 少直接碰 Telegram 发送 helper 和登录 service 装配细节。
- 在默认分支已经稳绿、固定 gate 已补齐的前提下，优先继续降低热点文件体积和渠道总调度耦合。
- 避免 `private_chat_runtime.py` 的主调度函数继续同时背负 shared routing 和 personal WeChat 渠道私有 wiring。

## Only do

- 只做一次保守瘦身：围绕 personal WeChat 登录路由抽出最小 helper 或 runtime。
- 保持现有 parser / routing / approval / SQLite 真相边界不变，不新增用户可感知功能，不改渠道外部协议。
- 保持当前登录回复文本、二维码发送方式、中文日志和现有 confirm / cancel / follow-up 语义不回退。
...
- 不改 personal WeChat 登录协议、二维码内容或 Telegram 发图/发文本接口语义。
...
当前 **`private_chat_runtime.py` 的 personal WeChat 登录路由边界瘦身** 主线视为 **已收口**，满足以下任一条即可：

1. personal WeChat 登录路由里的 service 读取、Telegram 发送 helper 读取或登录执行至少一段从 `private_chat_runtime.py` 抽离，且登录回复语义不变；
2. focused tests 能继续覆盖 personal WeChat 登录路由和当前 shared runtime 最小回归；
3. 默认分支全量 pytest 继续保持绿灯；
4. 文档继续保持分层一致，`STATUS.md` 只写当前快照，`NEXT_STEP.md` 只写当前唯一主线。

## After this step

1. 如果 personal WeChat 登录路由已收口一轮，就继续留在 `private_chat_runtime.py` 选下一条最小瘦身主线，或补对应 focused gate。
