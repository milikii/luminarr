...
- `app/services/manage_bt_subscription.py`
  - 当前状态：已再前推几格；CRUD 持久化 guard、扫描条目聚合 / 结果封装、scheduler tick chat 汇总、`last_seen` 回写护栏，以及单条订阅的搜索 / pending 创建编排都已下沉到 `bt_subscription_repo_support.py` / `bt_subscription_scan_support.py` / `bt_subscription_scheduler_support.py` / `bt_subscription_last_seen_support.py` / `bt_subscription_dispatch_support.py`；当前 `manage_bt_subscription.py` 为 `824` 行。
  - 目标：继续按“命令解析 / 剩余结果封装”拆分
