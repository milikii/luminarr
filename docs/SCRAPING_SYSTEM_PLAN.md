...
- `app/db/job_repo.py`
  - 当前状态：已再前推几格；身份校验、查询 helper，以及 lease/state/completed/cancel 写路径和 pending upsert SQL 都已下沉到 `job_repo_support.py`；当前又把 `mark_downloader_completed()` 的身份规范化继续下沉到 support helper，`job_repo.py` 为 `535` 行。
