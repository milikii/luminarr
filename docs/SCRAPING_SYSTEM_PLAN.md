...
- `app/services/subtitle_translator.py`
  - 当前状态：已再前推一格；support helper 已承接 chunk 翻译编排、异常校验、`ffprobe/ffmpeg` subprocess 执行编排、“可提取英文内嵌字幕结果筛选”、“提取后字幕文件可用性整理”，以及“单文件翻译读写边界”；当前 `subtitle_translator.py` 为 `492` 行，但热点已进一步收缩到翻译主流程本身
