...
## D-034 仓库同时提供“本地 Python”与“最小 Docker Compose”两条启动入口
- **状态**：已决定
- **日期**：2026-04-06
- **结论**：
...
## D-027 媒体型 BT 与后续追更必须预留独立名称解析步骤
- **状态**：已决定
- **日期**：2026-04-05
- **结论**：
  ...
  - 该步骤输出的最小结构化结果必须至少包含：
    ...
    - `episode`
    - `quality_tags`
  - 名称解析必须坚持 parser-first，优先使用确定性规则；后续允许增加项目内可配置的识别词 / 替换规则，用来处理动漫、国产剧和站点命名偏移，但当前不引入复杂 DSL。
...
## D-028 BT 选源后续升级为共享确定性评分器，不直接引入 DSL
- **状态**：已决定
- **日期**：2026-04-05
- **结论**：
...
## D-029 `job_event` 先做持久化事件账本，旁路 dispatcher 后续单独消费
- **状态**：已决定
- **日期**：2026-04-05
- **结论**：
...
## D-030 四个渠道当前都是正式私聊入口，但只维护一套共享业务真相
- **状态**：已决定
- **日期**：2026-04-05
- **结论**：
...
## D-031 cleanup 当前进入“有退出条件的验证窗口”，不做无限期观察
- **状态**：已决定
- **日期**：2026-04-05
- **结论**：
...
## D-032 文档按“决策 / 施工 / 现状 / 入口”分层，不重复堆叠同一事实
- **状态**：已决定
- **日期**：2026-04-05
- **结论**：
  ...
  - `README.md` 只保留项目定位、当前结构、能力概览和当前路线，作为仓库入口。
...
## D-033 项目交付形态继续以私聊 bot 为主，体验增强优先走渠道内 richer reply
- **状态**：已决定
- **日期**：2026-04-06
- **结论**：
