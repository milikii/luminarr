...
- **结论**：
  ...
  - 当前仍活跃的主线细节只保留在当前入口文档；已完成主线的 log / blueprint / focused tests / commit 轨迹统一移到 `archive/docs/`，更早完成的持久化收口细节继续保留在 `docs/PERSISTENCE_CLOSURE_LOG.md`，避免把 `docs/STATUS.md` 重新写胖。
  - `README.md` 只保留项目定位、当前结构、能力概览和当前路线，作为仓库入口；cleanup 窗口的详细证据仍收口到 `docs/CLEANUP_VERIFICATION_WINDOW.md`，已完成主线的详细收口台账统一放在 `archive/docs/`，不再长期占用主 `docs/` 目录。
...
## D-044 搜索与 BT 只读展示体验增强只作为后续候选主线，按 Telegram-first / 只读分阶段图源推进
- **状态**：已决定
- **日期**：2026-04-26
- **结论**：
  - 当前不切换 `docs/NEXT_STEP.md` 主线；“消息展示体验层”目前只作为后续候选主线，详细蓝图统一收口到 `docs/SEARCH_REPLY_PRESENTATION_PLAN.md`。
  - 这条后续主线首个 richer reply 渠道固定为 Telegram；Feishu / personal WeChat / WeCom 首阶段只要求继续复用 shared runtime，并输出更整洁的文本降级，不把四渠道富媒体能力绑成同一闭环。
  - 首轮展示增强的范围固定为：
    - 电影 / PT 搜索结果
    - `bt搜` 只读结果
    - `bt批量` 预览结果
    不把审批、状态、导入、cleanup 在同一轮一起扩成富媒体回包。
  - 电影搜索阶段的海报来源固定为只读在线图源；允许复用 TMDB / Fanart 查询能力，但不依赖导入后本地 sidecar 产物。
  - 成人 BT 的图片目标固定为“尽量全量带图”，但实现顺序必须分阶段：
    - 先做 exact-id 与稳定只读图源
    - 再扩到泛关键词 / 泛候选结果
    - 当候选与图源关系不稳定时，必须显式降级为纯文本，不得猜图或误贴
