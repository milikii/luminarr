# Release Prep Plan (v6)

> 目的：把当前阶段从“质量硬化”切到“收尾发布准备”后，后续最小闭环该按什么顺序推进写清楚。
>
> 当前定位：主功能已基本落地；现在的重点不是继续扩能力，而是把默认分支、文档真相、发布矩阵、真实 smoke 和质量 gate 收成一个可宣告首版发布准备完成的状态。

## 1. 当前阶段要解决什么

当前主要缺口不是“代码还没写完”，而是下面 4 件事还没完全收死：

1. `docs/STATUS.md`、`docs/NEXT_STEP.md`、operator prompt 仍残留旧阶段口径
...
目标：
- 同步 `STATUS` / `NEXT_STEP` / operator prompt 到当前真实复验结论
