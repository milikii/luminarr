...
- `*_PLAN.md` / `*_LOG.md` 只在它们仍承载当前真相、当前边界或当前测试入口时保留；完成态历史优先移到 `archive/docs/`，纯一次性记录再删除。
- `CODEX_*_PROMPT.md`、`*_PROMPTS.md` 这类纯工具提示词文件视为工具配置，不算项目文档；只要不再被引用，就优先删除。
- `logs/` 这类运行产物继续按忽略策略处理，不纳入项目真相。
