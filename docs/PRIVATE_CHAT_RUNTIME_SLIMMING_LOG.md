...
当前风险：
- shared runtime 还把 trace、`SERVICE_NOT_READY_TEXT`、渠道无关回复包装和部分 stop-path 文案揉在主函数里；这一步只允许把“生成回复文本 + 可观测性”收成 helper，不改日志协议和最终回复文本。
- 这一组继续守住“trace 只是补充观测，不替代中文失败日志”的边界。
...
- 当前唯一主线已经切到 `docs/PRIVATE_CHAT_RUNTIME_SLIMMING_LOG.md`；`docs/CLEANUP_SLIMMING_LOG.md` 转为已完成主线台账。
