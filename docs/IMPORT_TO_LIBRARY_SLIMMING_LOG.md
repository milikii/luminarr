# Import to library slimming log (v14)
...
剩余风险：
- context / approval / jobs / file-transfer / cancel / prepare / execution-tail / confirm-gate / confirm-expiry / event-recorder / raw_bt-guard / confirm-context-guard / metadata-title-year / pending-approval-write-through 十四段都已离开主文件；当前剩余最大块主要只剩 `event / pending job / approval` 薄 wrapper。
- 下一步先做 worth-it 复评估；只有还能带来真实边界收口，才继续动 `import_to_library.py` 或切去 `add_to_downloader.py`。
...
- 当前唯一主线已切到剩余 thin wrapper worth-it 复评估；本文件继续承接 import 微切分详细台账，不回到 dated 小节堆叠。
