# Import to library slimming log (v12)
...
剩余风险：
- context / approval / jobs / file-transfer / cancel / prepare / execution-tail / confirm-gate / confirm-expiry / event-recorder / raw_bt-guard / confirm-context-guard 十二段都已离开主文件；当前剩余最大块集中在 `_resolve_metadata_title_year()` 的 naming truth fallback glue，以及少量 event / pending wrapper。
- 下一步优先评估 metadata title-year helper；只动 metadata 入参收口，不回退 hardlink/copy-fallback、metadata、subtitle 或 refresh 协议。
...
- 当前唯一主线已切到 metadata title-year helper；本文件继续承接 import 微切分详细台账，不回到 dated 小节堆叠。
