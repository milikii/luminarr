# Import to library slimming log (v11)
...
剩余风险：
- context / approval / jobs / file-transfer / cancel / prepare / execution-tail / confirm-gate / confirm-expiry / event-recorder / raw_bt-guard 十一段都已离开主文件；当前剩余最大块集中在 `_rebuild_confirm_context()` 的 jobs/approval fail-closed 中文日志，以及 metadata title/year glue。
- 下一步优先评估 confirm context guard helper；只动 confirm context 判定收口，不回退 hardlink/copy-fallback、metadata、subtitle 或 refresh 协议。
...
- 当前唯一主线已切到 confirm context guard helper；本文件继续承接 import 微切分详细台账，不回到 dated 小节堆叠。
