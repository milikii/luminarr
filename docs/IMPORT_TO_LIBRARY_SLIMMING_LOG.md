# Import to library slimming log (v13)
...
剩余风险：
- context / approval / jobs / file-transfer / cancel / prepare / execution-tail / confirm-gate / confirm-expiry / event-recorder / raw_bt-guard / confirm-context-guard / metadata-title-year 十三段都已离开主文件；当前剩余最大块集中在 `_record_pending_approval()` 的 copy-fallback 清理 + approval write-through glue，以及少量 event / pending job wrapper。
- 下一步优先评估 pending approval write-through helper；只动 approval / copy-fallback glue 收口，不回退 hardlink/copy-fallback、metadata、subtitle 或 refresh 协议。
...
- 当前唯一主线已切到 pending approval write-through helper；本文件继续承接 import 微切分详细台账，不回到 dated 小节堆叠。
