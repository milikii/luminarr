...
- [x] 收口 `watchlist` 与 BT 专线边界：`watchlist sync` / `想看 同步` 当前保留显式命令，但只会 fail-closed 提示想看清单继续服务 PT 主线，不再把影视条目桥接进 `btsub`。

- [ ] 扩展成人 BT 专线的连续追踪边界：当前最小 contract 已落地，`btsub add` 已收口成成人 BT 精确番号追踪，旧的非成人订阅条目会显式告警并跳过扫描，同标题但不同 URL 的镜像命中不再重复创建下载待确认，`btsub list` 也会明确展示“上次命中资源”；后续若继续扩边，也只允许服务成人 BT 资源，不引入 raw BT / 影视资源订阅（包括动漫），不放宽自动确认，通知渠道也继续保持当前宿主链路边界。
