...
> **Luminarr v15 = 一个电影优先、Telegram 私聊唯一入口的垂直媒体自动化 Harness；它保留 TMDB-first 搜索、import-confirm、最小 lease/version 防重放等已落地能力，同时正式吸收“只读安全并发、LLM 物理异常响应式恢复、模糊查询探索隔离、审批唤醒上下文重建、挫败感短路”这五条工程纪律，并将下一阶段优先级重新拉回执行卫生与控制层。**
