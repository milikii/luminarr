...
> 目的：在已完成的用户页 / 列表页 / 编号范围页能力基础上，只补一个更小的“分类页 + 排序参数组合页 proof”，继续复用既有 approval -> confirm -> jobs 真相边界。
...
- 上一条“排序页 + `p=<页码>` 直达语法 proof”已经确认命令入口、页面抓取和聊天缓存都不回退；当前更小也更保守的缺口，只剩分类页与排序参数组合页的端到端 proof。
...
5. Phase 5：当前只补分类页与排序参数组合页的 focused proof；如果后续还要补更多站点或页面形式，再单独扩规则，不在这一条里顺手平台化。
...
- 当前只补 `https://nyaa.si/?c=1_2&s=seeders&o=desc` 这条分类排序组合页 proof；
...
1. `bt批量 https://nyaa.si/?c=1_2&s=seeders&o=desc 1-3` 已能返回只读批量预览，并证明命令入口会直接走页面抓取；对应 focused tests 全绿；
2. 上述组合页的候选已被证明可继续复用 `bt批量确认` 所需的聊天缓存边界，且未声明排序参数仍保持中文拒绝；对应 focused tests 全绿；
