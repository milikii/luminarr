...
- 首页翻页页 `https://nyaa.si/?p=2` 已落地，且仍走现有 allowlist 页面预览链；
- 排序列表页 `https://nyaa.si/?s=seeders&o=desc` 也已落地，且未声明参数仍保持 fail-closed；
- 当前这批 allowlist 列表页类型已收口；如果后续还要继续 BT，只再评估更多 allowlist 页面类型，不在这批里顺手扩站点。
