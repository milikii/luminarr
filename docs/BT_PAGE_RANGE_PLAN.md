...
> 目的：在已完成的分类列表显式分页 URL proof 基础上，再补一个更小的“分类基础页显式分页 URL” proof，继续复用既有 approval -> confirm -> jobs 真相边界。
...
- 上一条“分类列表显式分页 URL proof”已经确认命令入口、页面抓取、聊天缓存和 `bt批量确认` 复用都不回退；当前更小也更保守的缺口，只剩分类基础页的显式 `?c=1_2&p=2` URL 直链 proof，继续证明它也不会被打回关键词搜索。

## 2. 当前最小闭环

当前先只收一个更保守的入口：

1. 用户发送 `bt批量 https://nyaa.si/?c=1_2&p=2 1-3`；
...
30. Phase 30：当前只补分类基础页显式分页 URL 的 focused proof；如果后续还要补更多站点或页面形式，再单独扩规则，不在这一条里顺手平台化。
...
- `https://nyaa.si/?u=subsplease&s=seeders&o=desc&p=2` 已完成 focused proof，证明它仍走现有 allowlist 页面预览链，并继续复用 `bt批量确认` 边界；
- 当前只补 `https://nyaa.si/?c=1_2&p=2` 这条分类基础页显式分页 URL proof；
...
1. `bt批量 https://nyaa.si/?c=1_2&p=2 1-3` 已能返回只读批量预览，并证明命令入口会直接走页面抓取；对应 focused tests 全绿；
2. 上述分类基础页显式分页 URL 的候选已被证明可继续复用 `bt批量确认` 所需的聊天缓存边界；对应 focused tests 全绿；
