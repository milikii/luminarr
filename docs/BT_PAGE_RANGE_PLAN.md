...
> 目的：在已完成的分类排序组合页 proof 基础上，再补一个更小的“分类页 + 排序参数 + `p=<页码>` 语法” proof，继续复用既有 approval -> confirm -> jobs 真相边界。
...
- 上一条“分类页 + 排序参数组合页 proof”已经确认命令入口、页面抓取、聊天缓存和 `bt批量确认` 复用都不回退；当前更小也更保守的缺口，只剩分类排序组合页继续叠加 `p=<页码>` 语法的端到端 proof。
...
6. Phase 6：当前只补分类页 + 排序参数 + `p=<页码>` 语法的 focused proof；如果后续还要补更多站点或页面形式，再单独扩规则，不在这一条里顺手平台化。
...
- 当前只补 `https://nyaa.si/?c=1_2&s=seeders&o=desc p=2` 这条分类排序分页组合页 proof；
...
2. 上述分页组合页的候选已被证明可继续复用 `bt批量确认` 所需的聊天缓存边界，且未声明排序参数仍保持中文拒绝；对应 focused tests 全绿；
