# BT page / range plan (v11)

> 目的：在已完成的无分类用户排序组合页 proof 基础上，再补一个更小的“无分类用户排序分页组合页” proof，继续复用既有 approval -> confirm -> jobs 真相边界。
...
- 上一条“无分类用户排序组合页 proof”已经确认命令入口、页面抓取、聊天缓存和 `bt批量确认` 复用都不回退；当前更小也更保守的缺口，只剩无分类用户排序分页组合页继续证明 `p=<页码>` 语法不会把这个排序用户页打回关键词搜索。
...
20. Phase 20：当前只补无分类用户排序分页组合页的 focused proof；如果后续还要补更多站点或页面形式，再单独扩规则，不在这一条里顺手平台化。
...
- 当前只补 `https://nyaa.si/?u=subsplease&s=seeders&o=desc p=2` 这条无分类用户排序分页组合页 proof；
...
2. 上述无分类用户排序分页组合页的候选已被证明可继续复用 `bt批量确认` 所需的聊天缓存边界；对应 focused tests 全绿；
