...
> 目的：在已完成的用户页排序分页组合页 proof 基础上，再补一个更小的“搜索页 + 排序参数”组合页 proof，继续复用既有 approval -> confirm -> jobs 真相边界。
...
- 上一条“用户页 + 排序参数 + `p=<页码>` 语法 proof”已经确认命令入口、页面抓取、聊天缓存和 `bt批量确认` 复用都不回退；当前更小也更保守的缺口，只剩搜索页继续叠加排序参数的端到端 proof。

## 2. 当前最小闭环

当前先只收一个更保守的入口：

1. 用户发送 `bt批量 https://nyaa.si/?f=0&c=1_2&q=frieren&s=seeders&o=desc 1-3`；
...
9. Phase 9：当前只补搜索页 + 排序参数组合页的 focused proof；如果后续还要补更多站点或页面形式，再单独扩规则，不在这一条里顺手平台化。
...
- 当前只补 `https://nyaa.si/?f=0&c=1_2&q=frieren&s=seeders&o=desc` 这条搜索页排序组合页 proof；
...
1. `bt批量 https://nyaa.si/?f=0&c=1_2&q=frieren&s=seeders&o=desc 1-3` 已能返回只读批量预览，并证明命令入口会直接走页面抓取；对应 focused tests 全绿；
2. 上述搜索页排序组合页的候选已被证明可继续复用 `bt批量确认` 所需的聊天缓存边界，且未声明排序参数仍保持中文拒绝；对应 focused tests 全绿；
