...
```
项目名：Luminarr
技术栈：Python 3.12 / SQLite / python-telegram-bot / httpx / Docker Compose / Transmission / qBittorrent / Emby / Jellyfin / Plex / Telegram / personal WeChat / Feishu / WeCom
部署目标：单机、单进程、自托管影视自动化；Docker Compose 部署为主，本地 Python 运行调试为辅
仓库：git@github.com:milikii/luminarr.git
```
...
docs/INDEX.md          ← 文档分流入口（执行前先确认当前真相入口）
docs/STATUS.md         ← 当前实现与验证快照（执行前先确认是否仍是绿灯）
docs/NEXT_STEP.md      ← 当前唯一主线与禁止项（执行前先确认没有切错方向）
docs/PROGRESS.md       ← 进度日志（每轮追加，不得覆盖历史）
docs/BLOCKERS.md       ← 阻断问题（遇到时写入，停止并报告）
docs/DECISIONS.md      ← 执行中遇到的新决策点记录
```

启动检查：若以上文件缺失，停止执行并报告缺失文件，不得猜测内容继续推进。
PROGRESS.md 不存在时，创建并写入"Round 0: 项目初始化"后继续。
...
STEP 4 [验证]
  → 运行测试：`make quality` + `make verify-mainline`
  → 运行 lint：`make lint`
...
- 无孤立未调用的函数 / 类 / 变量
- 无注释掉的废弃代码块
- 每个公开函数有 docstring 说明意图
- 错误必须被捕获和处理，不允许裸露的 try/except pass
