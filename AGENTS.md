...
当前没有进行中的 promoted 主线；刚完成主线是 **cleanup PT 最小保护窗口**。2026-04-19 已确认：当 `pt_min_seed_hours` > 0 时，PT 任务会按 `download_monitor.completion_observed_at` 做保守阻断，必要真相缺失时 fail-closed；这一步不宣称已接入 downloader live seeding 秒数。后续若要继续做其他事项，先由用户把候选事项提升成新的 promoted 主线，再在 `docs/NEXT_STEP.md` 补齐 `Current goal / Only do / Done when`。已完成闭环入口继续看 `docs/JELLYFIN_PLEX_PLAN.md`、`docs/BT_SCORING_LOG.md`、`docs/QUICK_START_PLAN.md`、`docs/DEPLOY_CHECKLIST.md`、`docs/SHARED_DELIVERY_UX_LOG.md`、`docs/SERIES_ANIME_NAMING_LOG.md`、`docs/APP_MAIN_SLIMMING_LOG.md`、`docs/PRIVATE_CHAT_RUNTIME_SLIMMING_LOG.md`、`docs/CLEANUP_SLIMMING_LOG.md`、`docs/MANAGE_BT_SUBSCRIPTION_SLIMMING_LOG.md`、`docs/SEARCH_MEDIA_SLIMMING_LOG.md`、`docs/ADD_TO_DOWNLOADER_SLIMMING_LOG.md`、`docs/IMPORT_TO_LIBRARY_SLIMMING_LOG.md`、`docs/TELEGRAM_BOT_SLIMMING_LOG.md`、`docs/PERSISTENCE_CLOSURE_LOG.md` 与 `docs/CLEANUP_VERIFICATION_WINDOW.md`。
